...
if ($lista == "todos") {
	...
	$btVerificarPrespto = "display:none;";
	$btVerificarGasto = "display:none;";
}
...
<input type="hidden" name="FlagCajaChica" id="FlagCajaChica" />
<input type="hidden" name="FlagP" id="FlagP" />
<input type="hidden" name="FlagCC" id="FlagCC" />
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
            ...
            <input type="button" id="btVerificarGasto" value="Verificar CC" style="width:85px; <?=$btVerificarGasto?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=lg_requerimiento_form&opcion=verificarCC&origen=lg_requerimiento_lista', 'SELF', '', $('#registro').val());" />

            <input type="button" id="btVerificarPrespto" value="Verificar Partidas" style="width:110px; <?=$btVerificarPrespto?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=lg_requerimiento_form&opcion=verificarP&origen=lg_requerimiento_lista', 'SELF', '', $('#registro').val());" />
...
<table width="2130" class="tblLista">
	<thead>
    <tr>	
		...
		<th scope="col" width="60">Verif. CC</th>
		<th scope="col" width="60">Verif. Pr.</th>
		<th scope="col" width="60">Flag CC</th>
	</tr>
    </thead>
	...
	while ($field = mysql_fetch_array($query)) {
		...
		##
		?>
		<tr class="trListaBody" onclick="mClkVerificacion(this, 'registro', '<?=($field['FlagCajaChica'])?>' , '<?=($field['FlagP'])?>' , '<?=($field['FlagCC'])?>', '<?=($field['TipoClasificacion'])?>');" id="<?=$id?>">
...
<script language='JavaScript' type='text/JavaScript' >
	...
	document.getElementById('btConformar').disabled = true;
		document.getElementById('btVerificarGasto').disabled = true;
		document.getElementById('btVerificarPrespto').disabled = true
	
	function mClkVerificacion(src, registro,FlagCajaChica,FlagP,FlagCC, TipoClasificacion ) {
		mClk(src, registro);
// VERIFICAR SI ES DE COMPRAS		
if(TipoClasificacion == 'C') {	
		
				if(FlagCC == 'S') { // SI SE REALIZO EL CHEQUEO DE CAJA CHICA
					   if(FlagCajaChica == 'S') { // SI ES DE CAJA CHICA
						   
								if(FlagP == 'S') { // SI SE VERIFICO EL PRESUPUESTO
									
									document.getElementById('btConformar').disabled = false;
									document.getElementById('btVerificarGasto').disabled = true;  
									document.getElementById('btVerificarPrespto').disabled = true;  		   
								}else {
									...
									document.getElementById('btConformar').disabled = true;
									document.getElementById('btVerificarGasto').disabled = true;  
									document.getElementById('btVerificarPrespto').disabled = false;
								}
					 ...
					 document.getElementById('btConformar').disabled = true;
					 document.getElementById('btVerificarGasto').disabled = false;
					 document.getElementById('btVerificarPrespto').disabled = true
				 }
     ...
     /*var registro=document.getElementById(flagCC);
