...
<table width="850" class="tblLista">
    ...
    <tr>
		...
		<th scope="col" width="75">Verificado por Presupuesto</th>
		...
		$sql = "SELECT
			...
			FROM
				...
				LEFT JOIN seguridad_autorizaciones sa ON (sc.Concepto = sa.Concepto AND
														  ...
														  sa.Usuario = '".$_SESSION["USUARIO_ACTUAL"]."')
			WHERE sc.CodAplicacion = '".$_SESSION["APLICACION_ACTUAL"]."' AND sa.Concepto='01-0027'
			ORDER BY sc.Concepto";
	$query_autorizacion = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
	$field_autorizacion = mysql_fetch_array($query_autorizacion);
		if($field_autorizacion['FlagAgregar']=='N' or $field_autorizacion['FlagModificar']=='N' or $field_autorizacion['FlagEliminar']=='N')
		$disabled='disabled';
		?>
    ...
    while ($field_detalle = mysql_fetch_array($query_detalle)) {
        ...
        <tr class="trListaBody" onclick="mClk(this, 'sel_detalle');" id="detalle_<?=$nro_detalle?>">
            ...
            <td>
            <input type="checkbox" class="cell" name="FlagPresupuesto"  id="FlagPresupuesto_<?=$nrodetalle?>" <?=chkFlag($field_detalle['FlagPresupuesto'])?> <?=$visible?> <?=$disabled?> />
            </td>
            <td align="center">
