...
<table width="1100" class="tblForm">
    ...
    <tr>
		...
		<td>
        	<input type="text" id="MontoAfecto" value="<?=number_format($field_orden['MontoAfecto'], 2, ',', '.')?>" style="width:150px; text-align:right;" class="disabled" disabled="disabled" />
        </td>
	</tr>
    <tr>
    ...
    <tr>
		...
		<td>
        	<input type="text" id="MontoBruto" value="<?=number_format($field_orden['MontoBruto'], 2, ',', '.')?>" style="width:150px; text-align:right;" class="disabled" disabled="disabled" />
        </td>
	</tr>
    <tr>
		...
		<td>
        	<input type="text" id="MontoIGV" value="<?=number_format($field_orden['MontoIGV'], 2, ',', '.')?>" style="width:150px; text-align:right;" class="disabled" onfocus="numeroFocus(this);" onblur="numeroBlur(this);" onchange="setNuevoMontoIvaOC();" <?=$disabled_ver?> />
        </td>
	</tr>
    <tr>
		...
		<td>
        	<input type="text" id="MontoTotal" value="<?=number_format($field_orden['MontoTotal'], 2, ',', '.')?>" style="width:150px; text-align:right; font-size:12px; font-weight:bold;" class="disabled" disabled="disabled" />
        </td>
	</tr>
    <tr>
		...
		<td>
        	<input type="text" id="MontoPendiente" value="<?=number_format($field_orden['MontoPendiente'], 2, ',', '.')?>" style="width:150px; text-align:right;" class="disabled" disabled="disabled" />
        </td>
	</tr>
    <tr>
		...
		<td>
        	<input type="text" id="MontoOtros" value="<?=number_format($field_orden['MontoOtros'], 2, ',', '.')?>" style="width:150px; text-align:right;" class="disabled" disabled="disabled" />
        </td>
	</tr>
    <tr>
	...
	<tr>
		...
		<td colspan="3"><textarea id="Observaciones" style="width:95%; height:30px;" <?=$disabled_ver?>><?=($field_orden['Observaciones'])?></textarea></td>
	</tr>
	<tr>
...
<center>
  <?php
	...
	if($_GET['boton'] == 'verificarImputacion')
	{
		$tipoBoton = 'button';
		$llamado = 'mostrarVentanaVerificarImpuComp(this.form)';
		$label_submit = "Verificar Imputaci&oacute;n Correcta";
		$display_submit = "display:block;";
		
		$resul = $objConexion->consultar("select count(*) as cant from lg_verificarimpuordencom where Anio= '".$Anio."' and CodOrganismo='".$CodOrganismo."' and NroOrden='".$NroOrden."'",'fila');
		
		if ($resul['cant'] > 0)
		{
			echo "<script language=\"javascript\">
					alert('Ya se realizo la verificación de imputación correcta');
					 xGetElementById('frmentrada').submit();
					</script>";
		}
		
	} else if($_GET['boton'] == 'verificarPresupuesto')
	{
	
		$tipoBoton = 'button';
		$llamado = 'mostrarVentanaVerificarPresuComp(this.form);';
		$label_submit = "Verificar Presupuestariamente";
		$display_submit = "display:block;";
		
		$resul = $objConexion->consultar("select count(*) as cant from lg_verificarpresuordencom where Anio= '".$Anio."' and CodOrganismo='".$CodOrganismo."' and NroOrden='".$NroOrden."'",'fila');
		
		if ($resul['cant'] > 0)
		{
			echo "<script language=\"javascript\">
					alert('Ya se realizo la verificación presupuestaria');
					xGetElementById('frmentrada').submit();
					</script>";
		}
...
?>
  <table width="200" border="0">
    <tr>
      <td><input type="<?=$tipoBoton?>" value="<?=$label_submit?>" style=" <? echo $display_submit; ?>" onclick="<? echo $llamado; ?>" /></td>
      <td><input type="button" value="Cancelar" onclick="this.form.submit();" /></td>
      <td>&nbsp;</td>
    </tr>
  </table>
  <!--<input type="submit" value="<?=$label_submit?>" style=" <?=$display_submit?>" />--><!--<input type="button" value="Rechazar" onclick="orden_compra_rechazar(this.form);" style=" <?=$display_rechazar?>" />-->
</center>
