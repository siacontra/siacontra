...
// First : include the librairy
	...
	$resp2 = $objConexion->consultar($sql2,'matriz');
		...
		$sql1 = "SELECT p.Nombres, p.Apellido1, p.Apellido2, p.Ndocumento, pu.DescripCargo
	 	...
	 	$sql7 = "select A.FechaRegistro 	FROM
				 lg_controlperceptivo AS A
				WHERE A.CodControlPerceptivo = ".$CodControlPerceptivo;	
	...
	$anio = date('Y');
	*/
	list($anio, $mes, $diaNumero) = split("-", $resultado7['FechaRegistro']);
	$dia = $diaLetras[$diaNumero-1];		
//	$diaNumero = date('d');
	$mes = $mesLetra[$mes-1];
//	$anio = date('Y');
...
//	$odf->setVars('nroOrden',$nroOrden);
	...
	$odf->setVars('persona3',$resultado3['Nombres']." ".$resultado3['Apellido1']." ".$resultado3['Apellido2']);
	$odf->setVars('cedula3',$resultado3['Ndocumento']);	
	$odf->setVars('cargo3',$resultado3['DescripCargo']);
	
	$odf->setVars('persona4',$resultado4['Nombres']." ".$resultado4['Apellido1']." ".$resultado4['Apellido2']);
	$odf->setVars('cedula4',$resultado4['Ndocumento']);	
	$odf->setVars('cargo4',$resultado4['DescripCargo']);
