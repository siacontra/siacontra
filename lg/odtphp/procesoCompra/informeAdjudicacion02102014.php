...
// First : include the librairy
	...
	//	echo $resultado3['CodProveedor'];
		...
		$cadenaProveedor .= "- ".utf8_decode($resultado3['NomProveedor']).", RIF: ".$resultado3['Ndocumento'].", por un monto de Bs. ".number_format($resp5['total'],2,',','.').", ubicada en ".utf8_decode($resultado3['Direccion']).", ".utf8_decode($resultado3['Lnacimiento'])."
...
";
	...
	{
		$odf->setVars('tipoAdjudicacionSecuencia',utf8_decode('DECIDO OTORGAR LA ADJUDICACIÓN PARCIAL por medio del presente Informe a las empresas signadas con los Nros. '.$secuenciaRecomendacion.':'));
	
	} else {
		
		$odf->setVars('tipoAdjudicacionSecuencia',utf8_decode('DECIDO OTORGAR LA ADJUDICACIÓN por medio del presente Informe la empresa signada con el Nro. '.$secuenciaRecomendacion.':'));
	}	
	...
	$odf->setVars('objetoConsulta',utf8_decode($resultado7[0]['ObjetoConsulta']));
...
";
	...
	$odf->setVars('contralor',utf8_decode($resultadoOrganismo['RepresentLegal']));
	$odf->setVars('cedula',utf8_decode($resultadoOrganismo['DocRepreLeg']));
