...
function guardarGenerarActaInicio(registroCodSecGenerarActa,llamado,tipoReque)
{
	...
	var codAsistenteActaInicio2 = xGetElementById('asistenteActaInicio2');
	var codDirectorActaInicio = xGetElementById('directorActaInicio'); 
	var fechaReunion = xGetElementById('fr');
	var fechaIP = xGetElementById('fi'); 
	var fechaFP = xGetElementById('ff'); 
	var horaReunion = xGetElementById('hr'); 
	var PresupuestoBase = xGetElementById('pb');
	var fechaInv = xGetElementById('finv'); 
	var queryString = 'codAsistenteActaInicio='+codAsistenteActaInicio.value+'&codAsistenteActaInicio2='+codAsistenteActaInicio2.value+'&codDirectorActaInicio='+codDirectorActaInicio.value+'&tipoReque='+tipoReque+'&fechaReunion='+fechaReunion.value+'&horaReunion='+horaReunion.value+'&PresupuestoBase='+PresupuestoBase.value+'&fechaIP='+fechaIP.value+'&fechaFP='+fechaFP.value;
	var objCodActa;
	var form = null;
	var f=fechaInv.value;
	AF=f.split('-');
	var fR=fechaReunion.value;
	AFR=fR.split('-');
	...
	{
		alert('La fecha de la Reunión no puede ser Superior a la Fecha de la Invitación '+fechaInv.value);
		return;
	}
...
function anular_proc_CP()
{
	...
	var conf=confirm('Desea anular el procedimiento Nro. '+objCodActa.value);
	if(conf==true)
		{
		var motivo=prompt('Motivo de la anulación');
		}
	else
		return
	
	queryString = '&variableBusqueda='+objCodActa.value+'&an='+motivo;
	
	AjaxRequest.post
	(
		{
			'url':'lib/controladorCes.php',
			'parameters':{'caso':'anular_proc_CP'},
			'queryString':queryString,
			'onSuccess': function(req)
							{
								...
								if (req.responseText!=0)
								{
									...
									alert('No puede anularse el procedimiento');
								}
...
{ 	// se quito el llamado a la función 'devolverValorLista' ya que se cambio el combolist por un cuadro te texto
	...
	var pp=0;
	valorListaRenglonOf = xGetElementById('proveedorRenglonOf'+t).value;
	...
	puntajeCualitativoUno =(parseFloat(valorListaRenglonOf)+parseFloat(valorListaRequeTecUno)+parseFloat(valorListaTiempoEntregaUno)+parseFloat(valorListaCondicionPagoUno));
	
	xGetElementById('totalPuntuacionProvee'+t).value = puntajeCualitativoUno;
	
	xGetElementById('puntajeCuantitativo'+t).value = xGetElementById('pp1'+t).value;
...
{  // funcion modificada 20-02-2014 Guidmar Espinoza
	...
	var objAsistenteEvaluacion2= xGetElementById('asistenteEvaluacion2');
	...
	+'&asistenteEvaluacion2='+objAsistenteEvaluacion2.value
	...
	{ 
		...
		var valorListaRenglonOf = parseFloat(xGetElementById('proveedorRenglonOf'+i).value);
		...
		var items=xGetElementById('cantidadItems').value;
		var objCodProveedor = xGetElementById('codProveedor'+i).value;
		for(r = 1; r <= xGetElementById('cantidadItems').value; r++)
		{ 
			
		// se cambio a punto flotante, ya que el usuario puede ingresar valores decimales
		//alert(xGetElementById('totalPuntuacionProvee'+r+i).value);
		var objPMO_POE = parseFloat(xGetElementById('PMO_POE'+r+i).value);
		var objPP = parseFloat(xGetElementById('PP'+r+i).value);
		var valorSec = xGetElementById('secuencia'+r).value;
		var pRec = xGetElementById('pRec_'+r+i).value;
		...
		if(valorListaRenglonOf <'0')
		{
			alert('Existe puntaje sin seleccionar en la evaluación cualitativa (Renglones Ofertados)');
			return;
			break;	
		}
		if(valorListaCondicionPagoUno > '10')
		{
			alert('Existe puntaje mayor a 10 en la evaluación cualitativa (Condición de Pago)');
			return;
			break;	
		}
		
		if(valorListaTiempoEntregaUno > '10')
		{
			alert('Existe puntaje mayor a 10 en la evaluación cualitativa (Tiempo de Entrega)');
			return;
			break;	
		}
		
		if(valorListaRequeTecUno > '20')
		{
			alert('Existe puntaje mayor a 20 en la evaluación cualitativa (Requerimiento Técnico)');
			return;
			break;	
		}
		
		if(valorListaRenglonOf > '20')
		{
			alert('Existe puntaje mayor a 20 en la evaluación cualitativa (Renglones Ofertados)');
			return;
			break;	
		}
		
		if(xGetElementById('pRec_'+r+i).checked==true)
		pRec='S';
		else
		pRec='N';
		queryString+= '&items='+items+'&codProveedor['+i+']='+objCodProveedor
		+'&puntajeCualiTotal['+i+']='+objPuntajeCualiTotal
		+'&secuencia['+r+']='+valorSec
		+'&pRec['+r+i+']='+pRec
		+'&requeRenglonOf['+i+']='+valorListaRenglonOf
...
function guardarGenerarControlPerceptivo(llamado)
{
	...
	//&&(objPersona5.value == '')
	if ((objPersona1.value == '')&&(objPersona2.value == '')&&(objPersona3.value == '')&&(objPersona4.value == ''))
	{
		alert('Seleccione los 4 funcionarios');
		return;
	}
...
function guardarInformeRecomendacion()
{
	...
	var objNumeral = xGetElementById('numeral');
	var objPersona4 = xGetElementById('persona4');
	var objPersona5 = xGetElementById('persona5');
        var objPersona6 = xGetElementById('persona6');
	var form = null;
	var queryString = '';
	var t = 0;
	...
	{
		alert('Seleccione el Primer asistente');
		return;
	}
	
	if (objNumeral.value == '')
	{
		alert('Indique el Numeral');
		return;
	}
	
	if (objPersona5.value == '')
	{
		alert('Seleccione el Segundo asistente');
		return;
	}
        if (objPersona6.value == '')
	{
		alert('Seleccione el Tercer asistente');
		return;
	}
	...
	+'&persona[3]='+objPersona4.value
	+'&persona[4]='+objPersona5.value
        +'&persona[5]='+objPersona6.value
	+'&numeral='+objNumeral.value
	+proveedor;		
		//alert(queryString);
		//return;
		AjaxRequest.post
			...
			'onSuccess': function(req)
					{
						...
						//for(i = 0; i< 20; i++)
							pagina = pagina.replace(" ","");        
						window.open("evaluacion_anexo.php?codEvaluacion="+respuesta);
