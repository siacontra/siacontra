...
if ($accion == "commodity_detalle_insertar") {
    ...
    <td align="center">
     ...
     <td>
            <input type="checkbox" class="cell" id="FlagPresupuesto_<?=$nrodetalle?>" name="FlagPresupuesto" <?=chkFlag($field_detalle['FlagPresupuesto'])?> <?=$visible?> <?=$disabled?> />
            </td>
    <td align="center">
...
elseif($accion == "setActivosFijosAsociados") {
	...
	foreach ($detalle as $linea) {  //$_CantidadPendiente,
		list($_CommoditySub, $_Descripcion, $_CodUnidad, $_CantidadPedida,  $_CantidadRecibida, $_CantidadPendiente,$_Monto, $_CodClasificacion, $_CodCentroCosto, $_Secuencia) = split(";char:td;", $linea);
