...
//	FUNCION PARA CARGAR UNA NUEVA VENTANA
function cargarPDF2(form, pag, param) {
	...
	{
		var pagina=pag+"?registro="+codigo;		
		window.open(pagina, "", "toolbar=no, menubar=no, location=no, scrollbars=yes, width=600,height=600");
	}
}
...
//	FUNCION QUE MANEJA LOS EVENTOS PARA ELIMINAR
function controlador(form, pagina, foraneo, modulo,url) {
	var codigo=form.registro.value;
	if (codigo=="") msjError(1000);
	else {
		//	PREGUNTO SI DESEA ELIMINAR
...
//	FUNCION QUE MANEJA LOS EVENTOS PARA ELIMINAR
function hcmPDFSolicitud(form) {
	var codigo=form.registro.value;
	if (codigo=="") msjError(1000);
      
   //   cargarPagina(form, pagina);
	cargarVentana(form, 'hcm_planilla_pdf.php?codigo='+codigo, 'height=800, width=800, left=200, top=200, resizable=yes');
...
//	FUNCION PARA BLOQUEAR/DESBLOQUEAR
function enabledGInstrucc2(form) {
	if (form.chkprofesion.checked) form.fprofesion.disabled=false; 
	else { form.fprofesion.disabled=true; form.fprofesion.value=""; }
}
...
function selEmpleado(busqueda, campo) {
	...
	else if (campo == 4) { //para buscar la carga familiar del empleado seleccionado 
		opener.document.frmentrada.codempleado.value=registro;
		opener.document.frmentrada.nomempleado.value=busqueda;
		
		cargarFamiliarEmpleado(registro,'CARGAFAMILIAR');
		
	}  else if (campo == 5) { //para buscar los hijos del funcionario seleccionado 
		opener.document.frmentrada.codempleado.value=registro;
		opener.document.frmentrada.nomempleado.value=busqueda;	
		cargarHijosEmpleado(registro,'CARGAHIJOS');		
	}
...
function cargarHijosEmpleado(codigo, accion)
{
	...
	AjaxRequest.post
		...
		,'url':url
		,'onSuccess': function(req){respCargarHijosEmpleado(req,codigo)}
...
function respCargarHijosEmpleado(req,codigo)
{
	//alert('sdfsdfsdfsdf');
	...
	//var parentesco ='';
		...
		window.opener.xGetElementById('sel_familia_funcionario').length=1;	
		window.opener.xGetElementById('sel_familia_funcionario').options[0].value = '0';
		window.opener.xGetElementById('sel_familia_funcionario').options[0].text  = '..';
	
	
	//var OBJ_BENEFICIO_UTILES=new itemHijo();
	if(resp!=null)
	{
		window.opener.xGetElementById('sel_familia_funcionario').length=resp.length+1;
		...
		for(var i=0; i<resp.length;i++)
		{
			...
			*/
			...
			window.opener.xGetElementById('sel_familia_funcionario').options[i+1].value=resp[i]['CodSecuencia']+'-'+resp[i]['NombresCarga'];
			window.opener.xGetElementById('sel_familia_funcionario').options[i+1].text=resp[i]['NombresCarga'];	
			//window.opener.xGetElementById('sel_familia_funcionario').options[i+1].disabled=disabled;						
	...
	window.close();
	//buscarConsulidoFuncionario(codigo);
...
function cargarFamiliarEmpleado(codigo, accion) {
												...
												(
								       				{
								            			...
								            			,'url':url
								            			,'onSuccess': function(req){respCargarFamiliarEmpleado(req,codigo)}
														,'onError':function(req)
														{ 
								                        	alert('Error!\nStatusText='+req.statusText+'\nContents='+req.responseText);
								                        }         
...
function respCargarFamiliarEmpleado(req,codigo)
{
	//alert('sdfsdfsdfsdf');
	var resp =eval ("("+req.responseText+")");
	//alert(resp);
	...
	if(resp!=null)
	{
		...
		for(var i=0; i<resp.length;i++)
		{
			...
			window.opener.xGetElementById('sel_familia_funcionario').options[i+1].value=resp[i]['CodSecuencia'];
			window.opener.xGetElementById('sel_familia_funcionario').options[i+1].text=resp[i]['NombresCarga']+'      --      '+parentesco;
		}
	}
	buscarConsulidoFuncionario(codigo);
...
//03-12/2012
function buscarConsulidoFuncionario(codigo)
{
	//alert(codigo);
	var opcion = 'CONSUMIDO';
	var url = 'lib/transBeneficio.php';
	AjaxRequest.post
												(
								       				{
								            			...
								            			,'url':url
								            			,'onSuccess': respBuscarConsulidoFuncionario												       
														,'onError':function(req)
														{ 
								                        	alert('Error!\nStatusText='+req.statusText+'\nContents='+req.responseText);
								                        }         
...
function respBuscarConsulidoFuncionario(req)
{
	//alert('dfdsf');
	var consumido  = '';
	var resp =eval ("("+req.responseText+")");
	//alert(resp[0]['CONSUMIDO']);
	if(resp[0]['CONSUMIDO']==null)
		consumido  = '0,00';
	else
		consumido = toCurrency(resp[0]['CONSUMIDO']);
		
	window.opener.xGetElementById('td_consumido').innerHTML=consumido;
...
function eliminarBeneficio(form, pagina, foraneo, modulo) {
	var idCodigo=document.getElementById("registro");	var codigo = new String (idCodigo.value);
	...
	if (codigo=="") msjError(1000);
		...
		if (eliminar) {
			...
			ajax.open("POST", "fphp_ajax.php", true);
			ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			ajax.send("modulo="+modulo+"&accion=ELIMINAR&codigo="+registro);
			ajax.onreadystatechange=function() {
				if (ajax.readyState==4)	{
					var error=ajax.responseText;
					if (error!=0) alert ("¡"+error+"!");
					else cargarPagina(form, "beneficioListar.php");
				}
			}		
		}
	}
...
function anularBeneficio(form, pagina){
	...
	if (codigo=="") msjError(1000);
		...
		var anular=confirm("¡Esta seguro de anular el beneficio?");
		if (anular) {
			//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
			var ajax=nuevoAjax();
			ajax.open("POST", "fphp_ajax.php", true);
			ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			ajax.send("modulo=BENEFICIO&accion=ANULAR&codigo="+codigo);
			ajax.onreadystatechange=function() {
				if (ajax.readyState==4)	{
					var error=ajax.responseText;
					if (error!=0) alert ("¡"+error+"!");
...
//	FUNCION PARA ELIMINAR UN BENEFICIO UTILES
function eliminarBeneficioUtiles(form, pagina, foraneo, modulo) {
	var idCodigo=document.getElementById("registro");	var codigo = new String (idCodigo.value);
	//var elemento = form.det.value;
	if (codigo=="") msjError(1000);
	else {
		...
		if (eliminar) {
			...
			ajax.open("POST", "fphp_ajax.php", true);
			ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			ajax.send("modulo="+modulo+"&accion=ELIMINARUTILES&codigo="+registro);
			ajax.onreadystatechange=function() {
				if (ajax.readyState==4)	{
					var error=ajax.responseText;
					if (error!=0) alert ("¡"+error+"!");
					else cargarPagina(form, "listarBeneficiosUtiles.php");
				}
			}		
		}
	}
...
// 28-03-2014 CARLOS MARCANO (PRUEBAS) HCM FUNCION PARA VERIFICAR LOS DATOS DE SOLICITUD PARA PLANILLAS
function verificarSolicitudHCM(form, accion) {
	var codempleado = new String (form.codempleado.value); 
		codempleado=codempleado.trim();
		form.codempleado.value=codempleado;
		
		//salert(codempleado);
	//	VERIFICO QUE LOS CAMPOS OBLIGATORIOS NO ESTEN VACIOS
	if (codempleado=="") msjError(1010);
	else if(codempleado=='0') msjError(1010);
	else {
		//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
		var ajax=nuevoAjax();
		ajax.open("POST", "lib/SolicitudControlador.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("modulo=HCM&accion="+accion+"&codfuncionario="+codempleado+"&codigo=");
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				var error=ajax.responseText;
				if (error!=0) alert ("¡"+error+"!");
				else location.href="hcm_planilla.php?filtro="+form.filtro.value;
			}
		}
	}
	return false;
}
// 25-03-2014 CARLOS MARCANO (PRUEBAS) HCM FUNCION PARA VERIFICAR LOS DATOS DE LOS MEDICOS
function verificarMedicos(form, accion) {
	var nombremedico = new String (form.txt_nombremedico.value); 
		nombremedico=nombremedico.trim();
		form.txt_nombremedico.value=nombremedico;
		
	var telefono = new String (form.txt_telefono.value); 
		telefono=telefono.trim();
		form.txt_telefono.value=telefono;
		
		
	//	VERIFICO QUE LOS CAMPOS OBLIGATORIOS NO ESTEN VACIOS
	if (txt_nombremedico=="") msjError(1010);
	else if(txt_telefono=='0') msjError(1010);
	else {
		//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
		var ajax=nuevoAjax();
		ajax.open("POST", "lib/MedicosControlador.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("modulo=HCM&accion="+accion+"&telefono="+telefono+"&nombremedico="+nombremedico+"&codigo="+form.txt_codigo.value);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				var error=ajax.responseText;
				if (error!=0) alert ("¡"+error+"!");
				else location.href="hcm_medicos.php?filtro="+form.filtro.value;
			}
		}
	}
	return false;
}
// 25-03-2014 CARLOS MARCANO (PRUEBAS) HCM FUNCION PARA VERIFICAR LOS DATOS DE LAS INSTITUCIONES HCM
function verificarInstituciones(form, accion) {
	
	var descripcioninsthcm = new String (form.txt_descripcioninsthcm.value); 
		descripcioninsthcm=descripcioninsthcm.trim();
		form.txt_descripcioninsthcm.value=descripcioninsthcm;
		
	var telefonos = new String (form.txt_telefonos.value); 
		telefonos=telefonos.trim();
		form.txt_telefonos.value=telefonos;
		
	var direccion = new String (form.txt_direccion.value); 
	    direccion=direccion.trim();
	    form.txt_direccion.value=direccion;
	
	//	VERIFICO QUE LOS CAMPOS OBLIGATORIOS NO ESTEN VACIOS
	if (txt_descripcioninsthcm=="") msjError(1010);
	else if(txt_telefonos=="") msjError(1010);
	else if(txt_direccion=="") msjError(1010);
	
	else {
		//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
		var ajax=nuevoAjax();
		ajax.open("POST", "lib/InstitucionesControlador.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("modulo=HCM&accion="+accion+"&direccion="+direccion+"&telefonos="+telefonos+"&descripcioninsthcm="+descripcioninsthcm+"&codigo="+form.txt_codigo.value);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				var error=ajax.responseText;
				if (error!=0) alert ("¡"+error+"!");
				else location.href="hcm_instituciones.php?filtro="+form.filtro.value;
			}
		}
	}
	return false;
...
//	MAESTRO DE ASIGNACIÓN DE BENEFICION GENERAL 21-11-2012 ERNESTO RIVAS
//	FUNCION PARA VERIFICAR QUE SE INGRESARON LOS DATOS OBLIGATORIOS Y QUE EL REGISTRO NO EXISTA EN LA BASE DE DATOS
function verificarAsignacionGlobal(form, accion) {
	var descripcion = new String (form.txt_descripcion.value); 
		descripcion=descripcion.trim();
		form.txt_descripcion.value=descripcion;
	//	VERIFICO QUE LOS CAMPOS OBLIGATORIOS NO ESTEN VACIOS
	if (descripcion=="") msjError(1010);
		...
		ajax.open("POST", "fphp_ajax.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("modulo=ASIGNACIONGLOBAL&accion="+accion+"&codigo="+form.txt_codigo.value+"&descripcion="+descripcion+"&limite="+form.txt_limite.value);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				var error=ajax.responseText;
				if (error!=0) alert ("¡"+error+"!");
				else location.href="asignacion_global.php?filtro="+form.filtro.value;
			}
		}
	}
	return false;
...
//	MAESTRO DE ASIGNACIÓN DE BENEFICION GENERAL 21-11-2012 ERNESTO RIVAS
//	FUNCION PARA VERIFICAR QUE SE INGRESARON LOS DATOS OBLIGATORIOS Y QUE EL REGISTRO NO EXISTA EN LA BASE DE DATOS
function verificarAsignacion(form, accion) {
	var descripcion = new String (form.txt_descripcion.value); 
		descripcion=descripcion.trim();
		form.txt_descripcion.value=descripcion;
		
	var asigGlobal = new String (form.sel_asignacionGeneral.value); 
		asigGlobal=asigGlobal.trim();
		form.sel_asignacionGeneral.value=asigGlobal;
		
	var limite = new String (form.txt_limite.value); 
		limite=limite.trim();
		form.txt_limite.value=limite;
		
		
	var personaAprobar = new String (form.codempleado.value); 
		personaAprobar=personaAprobar.trim();
		form.codempleado.value=personaAprobar;
		
	//	VERIFICO QUE LOS CAMPOS OBLIGATORIOS NO ESTEN VACIOS
	if (descripcion=="") msjError(1010);
	else if(asigGlobal=='0') msjError(1010);
	else if(limite=="0,00") msjError(1010);
	else if(personaAprobar=="") msjError(1010);
	else {
		//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
		var ajax=nuevoAjax();
		ajax.open("POST", "fphp_ajax.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("modulo=ASIGNACIONESPECIFICO&accion="+accion+"&codigo="+form.txt_codigo.value+"&descripcion="+descripcion+"&limite="+form.txt_limite.value+"&personaAprobar="+personaAprobar+"&asigGlobal="+asigGlobal);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				var error=ajax.responseText;
				if (error!=0) alert ("¡"+error+"!");
				else location.href="asignacion_beneficio.php?filtro="+form.filtro.value;
			}
		}
	}
	return false;
...
function verificarGrupoOcupacional(form, accion) {
	...
	//	VERIFICO QUE LOS CAMPOS OBLIGATORIOS NO ESTEN VACIOS
	if (codigo == "" || descripcion == "") msjError(1010);
	else if (codigo.length != 5) alert("¡EL CODIGO DEBE TENER 5 DIGITOS!");
	else {
		//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
		var ajax=nuevoAjax();
		ajax.open("POST", "fphp_ajax.php", true);
...
function verificarCargo(form, accion) {
	...
	else if (codcargo.length != 5) ("¡EL CODIGO DEBE TENER 5 DIGITOS!")
	else {
		//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
		var ajax=nuevoAjax();
		alert
...
function filtroPDFEmpleados(form, limit) {
	...
	if (form.chkprofesion.checked) filtro+=" AND ei.CodGradoInstruccion = *"+form.fprofesion.value+"*";
	if (form.chkpersona.checked) {
		var rel="";
		if (form.sltpersona.value=="1") rel="=";
		else if (form.sltpersona.value=="2") rel="<";
...
function filtroPDFEventosControl2(form, limit) {
	var filtro=""; var ordenar="ORDER BY me.CodEmpleado";
	if (form.chkedoreg.checked) filtro+=" AND mp.Estado LIKE *;"+form.fedoreg.value+";*";
	if (form.chkorganismo.checked) filtro+=" AND me.CodOrganismo LIKE *;"+form.forganismo.value+";*";
	if (form.chkdependencia.checked) filtro+=" AND en.CodDependencia LIKE *;"+form.fdependencia.value+";*";
	...
	if (esFIngD && esFIngH) {
		if (form.chkbuscar.checked && form.sltbuscar.value=="") msjError(1130);
			...
			if (error) alert("¡DEBE SELECCIONAR EL PERIODO DEL REPORTE!");
				...
				form.target = "iAsistencias";				
				cargarPagina(form, pagina_asistencias);
				var pagina_permisos="pdf_eventos_permisos2.php?filtro="+filtro+"&filtro_permiso="+filtro_permiso+"&filtro_permiso="+filtro_permiso+"&fingresod="+form.ffingresod.value+"&fingresoh="+form.ffingresoh.value;
