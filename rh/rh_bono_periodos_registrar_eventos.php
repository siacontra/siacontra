    ...
    <tr>
    	<td>
			...
			for($ns=1;$ns<=$nro_semanas;$ns++) {
                ?>
                <li id="li<?=$ns?>" onclick="currentTab('tab', this);currentTab('tabL', this);" class="<?=$current[$ns]?>">
                    <a href="#" onclick="mostrarTab('tab', <?=$ns?>, <?=$nro_semanas?>);mostrarTab('tabL', <?=$ns?>, <?=$nro_semanas?>);"><?=$ttl[$ns]?></a>
                </li>
                <?
            }
            ?>
...
<!-------------------------------------------------------------------------------->
<!---------------------------------------------------------------------------------------------------------------->
<input type="hidden" name="sel_laborados" id="sel_laborados" />
<?php
//	tabs
if ($nro_tab == "") $nro_tab = 1;
for($ns=1;$ns<=$nro_semanas;$ns++) {
	if ($nro_tab == $ns) $display = "block"; else $display = "none";
	?>
    <div id="tabL<?=$ns?>" style="display:<?=$display?>;">
    <table width="950" class="tblBotones">
        <tr class="gallery clearfix">
        	<td>
				
				Registrar Fines de Semana Laborados

            </td>
            <td align="right">
                <input type="button" class="btLista" value="Insertar" onclick="bono_periodos_registrar_laborados_insertar(this, '<?=$ns?>', '<?=$fechai[$ns]?>', '<?=$fechaf[$ns]?>');" />
                <input type="button" class="btLista" value="Borrar" onclick="bono_periodos_registrar_laborados_quitar(this, '<?=$ns?>');" />
            </td>
        </tr>
    </table>
    <div style="overflow:scroll; width:950px; height:250px;">
    <table width="100%" class="tblLista">
        ...
        <tbody id="lista_laborados_<?=$ns?>">
		<?php
        //	consulto
        $sql = "SELECT *
                FROM rh_bonoalimentacionlaborados
                WHERE
					Anio = '".$Anio."' AND
					CodOrganismo = '".$CodOrganismo."' AND
					CodBonoAlim = '".$CodBonoAlim."' AND
        ...
        while($field_eventos = mysql_fetch_array($query_eventos)) {
            ?>
            <tr class="trListaBody" onclick="mClk(this, 'sel_laborados');" id="laborados_<?=$ns?>_<?=$nro_eventos?>">
                <th>
                    <?=++$nro_eventos?>
                </th>
                <td>
                    <select name="laborados_Fecha" id="Fecha_<?=$ns?>_<?=$nro_eventos?>" class="cell" onChange="getDiffHoraEventos('<?=$ns?>_<?=$nro_eventos?>');">
                        <?=getFechaEventos($fechai[$ns], $fechaf[$ns], formatFechaDMA($field_eventos['Fecha']), 0)?>
                    </select>
                </td>
             
                <td>
                    <textarea name="laborados_Observaciones" class="cell" style="height:15px;"><?=$field_eventos['Observaciones']?></textarea>
                </td>
            </tr>
            <?
        }
    ...
    <input type="hidden" id="nro_laborados_<?=$ns?>" value="<?=$nro_eventos?>" />
    <input type="hidden" id="can_laborados_<?=$ns?>" value="<?=$nro_eventos?>" />
    </div>
    </div>
	<?
}
