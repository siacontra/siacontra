...
<?php echo"<form name='frmentrada' action='contratos_porvencer.php?filtro=".$_GET['filtro']."' method='POST'>"; ?>
...
// CONSULTO SI EXISTE EL PARAMETRO DE DIAS DE FIN DE CONTRATO SINO LO INICIALIZO EN 1
$sql="SELECT ValorParam FROM mastparametros WHERE ParametroClave='VENCON' AND TipoValor='N' AND Estado='A'";
$query=mysql_query($sql) or die ($sql.mysql_error());
$rows=mysql_num_rows($query);
if ($rows!=0) {$field=mysql_fetch_array($query); $PAR_DIASFINCON=(int) $field[0];} else $PAR_DIASFINCON=1;
	...
	<tr>
		...
		<td width="300" align="center">
			...
			<table align='center'>
				<tr>
					<td>
						<input name='btPrimero' type='button' id='btPrimero' value='&lt;&lt;' onclick='setLotes(this.form, \"P\", $registros);' />
						<input name='btAtras' type='button' id='btAtras' value='&lt;' onclick='setLotes(this.form, \"A\", $registros);' />
					</td>
					<td>Del</td><td><div id='desde'></div></td>
					<td>Al</td><td><div id='hasta'></div></td>
					<td>
						<input name='btSiguiente' type='button' id='btSiguiente' value='&gt;' onclick='setLotes(this.form, \"S\", $registros);' />
						<input name='btUltimo' type='button' id='btUltimo' value='&gt;&gt;' onclick='setLotes(this.form, \"U\", $registros);' />
					</td>
				</tr>
			</table>";
			?> 
		...
		<td width="500" align="right">
			<input name="btNuevo" type="button" id="btNuevo" class="btLista" value="Nuevo" />
			<input name="btEditar" type="button" id="btEditar" class="btLista" value="Editar" />
			...
			<input name="btAbrir" type="button" id="btAbrir" class="btLista" value="Abrir" />
...
$sql="SELECT rc.CodPersona, mp.NomCompleto, rtc.Descripcion AS TipoContrato, me.Fingreso, me.CodEmpleado, rp.DescripCargo, mo.Organismo, rc.FechaDesde, rc.FechaHasta, rc.FlagFirma, rc.FechaFirma, DATEDIFF(rc.FechaHasta, NOW()) AS Faltan FROM rh_contratos rc INNER JOIN mastpersonas mp ON (rc.CodPersona=mp.CodPersona) LEFT JOIN rh_tipocontrato rtc ON (rc.TipoContrato=rtc.TipoContrato) INNER JOIN mastempleado me ON (rc.CodPersona=me.CodPersona) INNER JOIN mastorganismos mo ON (me.CodOrganismo=mo.CodOrganismo) INNER JOIN rh_puestos rp ON (me.CodCargo=rp.CodCargo) INNER JOIN seguridad_alterna sa ON (me.CodDependencia=sa.CodDependencia AND sa.CodAplicacion='".$_SESSION["APLICACION_ACTUAL"]."' AND sa.Usuario='".$_SESSION["USUARIO_ACTUAL"]."' AND sa.FlagMostrar='S') WHERE rc.Estado='VI' AND (DATEDIFF(rc.FechaHasta, NOW())<=$PAR_DIASFINCON) $filtrado ORDER BY rc.CodPersona LIMIT ".$_GET['limit'].", $MAXLIMIT";
	...
	<table width='1000' class='tblLista'>
	  <tr class='trListaHead'>
			...
			<th width='275' scope='col'>Nombre Completo</th>
			<th width='75' scope='col'>Tipo de Contrato</th>
			...
			<th width='100' scope='col'>Vence</th>
		</tr>";
	for ($i=0; $i<$rows; $i++) {
		...
		<tr class='trListaBody' onclick='mClk(this, \"registro\");' onmouseover='mOvr(this);' onmouseout='mOut(this);' id='".$field['CodPersona']."'>
			<td align='center'><input type='checkbox' name='sel' value='".$field['CodPersona']."' id='i_".$field['CodPersona']."' /> </td>
...
<script type='text/javascript' language='javascript'>
	totalContratos(3, $registros, \"$_ADMIN\", \"$_INSERT\", \"$_UPDATE\", \"$_DELETE\");
