...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Aprobar Jubilaci&oacute;n</td>
...
if ($filtrar=="DEFAULT") {
	...
	$filtro="AND (me.CodOrganismo=*".$_SESSION["FILTRO_ORGANISMO_ACTUAL"]."* AND me.CodDependencia=*".$_SESSION["FILTRO_DEPENDENCIA_ACTUAL"]."* AND me.Estado=*".$_POST['fsittra']."* AND mp.Estado=*".$_POST['fedoreg']."*)";
}
if ($accion == "FILTRAR") $filtro = $filtrar;
...
//
echo "
<form name='frmentrada' action='jubilacion_aprobar_listado.php?filtro=".$filtro."' method='POST' onsubmit='return false'>
...
<table width='1000' class='tblFiltro'>
	<tr>
		...
		<td>
			<input type='checkbox' name='chkedoreg' id='chkedoreg' value='1' $obj[3] onclick='forzarCheck(\"chkedoreg\");' />
	...
	<tr>
		...
		<td>
			<input type='checkbox' name='chksittra' id='chksittra' value='1' $obj[10] onclick='forzarCheck(\"chksittra\");' />
...
<center><input type='button' name='btBuscar' value='Buscar' onclick='filtroAprobarJubilacion(this.form, ".$limit.");'></center>
...
$sql="SELECT 
			rpj.CodPersona, 
			mp.Estado AS EsReg, mp.NomCompleto, mp.Ndocumento, 
			me.CodEmpleado, me.Fingreso, me.Estado AS SitTra, 
			md.Dependencia, 
			rmc.FlagFaltaGrave 
		FROM 
		...
		<td valign="top">
			<table width="1000" class="tblBotones">
			  <tr>
			    ...
			    <td align="right">
						<input name="btPDF" type="button" id="btPDF" value="Ver Reporte PDF" onclick="pdfJubilacionSel();" /> | 
						<input name="btAprobar" type="button" id="btAprobar" value="Aprobar Jubilaci&oacute;n" onclick="cargarOpcionJubilacion(this.form, 'jubilacion_aprobar_abrir.php?accion=APROBAR', 'ABRIR-APROBAR');" />
					</td>
			  </tr>
			</table>
			...
			<table width="1000" class="tblLista">
				<tr class="trListaHead">
					...
					<th width="350" scope="col">Dependencia</th>
				</tr>
				<?php 
				if ($registros!=0) {
					//	CONSULTO LA TABLA
				...
				<script type='text/javascript' language='javascript'>
					totalRegistrosA($registros, \"$_ADMIN\", \"$_INSERT\", \"$_UPDATE\", \"$_DELETE\");
