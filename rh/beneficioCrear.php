...
<tr>
 <td class="titulo">Beneficio | Nuevo</td>
...
$anho = date('Y');
$sqlN_HCM="SELECT MAX( nroBeneficio )+1 AS NRO FROM rh_beneficio WHERE anhoEjecucio ='".$anho."'";
...
<div style="width:800px" class="divFormCaption">Informaci&oacute;n de Beneficio</div>
...
<tr>
 ...
 <select name="sel_servicio" id="sel_servicio" onchange="colocarResponsable();calcularConsumo(value)" style="width:50%">
   </select>
   *</td>
 <td align="left">&nbsp;</td>
 </tr>
...
<tr>
  ...
  <td colspan="2"><select name="sel_institucion" id="sel_institucion" style="width:80%" >
    </select>
    *</td>
  <td>&nbsp;</td>
  </tr>
<tr>
  ...
  <td colspan="2"><select name="sel_medico" id="sel_medico" style="width:80%">
    </select>
    *</td>
  <td>&nbsp;</td>
  </tr>
...
<tr>
  ...
  <td class="tagForm">Preparado por:</td><? $sql3=mysql_query("SELECT * FROM usuarios WHERE Usuario='".$_SESSION['USUARIO_ACTUAL']."'");
   ...
   <td><input name="prepor" id="prepor" type="text" size="60" value="<?=$field4['NomCompleto']?>" readonly/>
    	<input name="codprepor" type="hidden" id="codprepor" value="<?=$_SESSION['USUARIO_ACTUAL'];?>" /><? echo $_SESSION['USUARIO_ACTUAL'];   ?></td>
<tr>
<tr><td></td>
...
<center>
<input name="btguardar" type="submit" id="btguardar" value="Guardar Registro" onclick="validarCampoBeneficio()"/>
...
function respCargarServicio(req)
{
												...
												(
								       				{
								            			...
								            			,'url':url
								            			,'onSuccess': respCargarInstitucion													       
														,'onError':function(req)
														{ 
								                        	alert('Error!\nStatusText='+req.statusText+'\nContents='+req.responseText);
								                        }         
...
function respCargarInstitucion(req)
{
