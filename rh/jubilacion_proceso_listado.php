...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Proceso de Jubilaci&oacute;n</td>
...
//
echo "
<form name='frmentrada' action='jubilacion_proceso_listado.php?filtro=".$filtro."' method='POST' onsubmit='return false'>
...
<center><input type='button' name='btBuscar' value='Buscar' onclick='filtroEmpleadosJubilacion(this.form, ".$limit.");'></center>
		...
		<td valign="top">
			<table width="1000" class="tblBotones">
			  <tr>
			    ...
			    <td align="right">
						...
						<input name="btProcesar" type="button" id="btProcesar" value="Procesar Jubilaci&oacute;n" onclick="cargarOpcionJubilacion(this.form, 'jubilacion_proceso_abrir.php?accion=PROCESAR', 'ABRIR-PROCESAR');" />
					</td>
			  </tr>
			</table>
			...
			<table width="1000" class="tblLista">
				...
				if ($registros!=0) {
					...
					$sql="SELECT mp.Estado AS EsReg, mp.CodPersona, mp.NomCompleto, mp.Ndocumento, me.CodEmpleado, me.Fingreso, me.Estado AS SitTra, md.Dependencia, rmc.FlagFaltaGrave FROM mastpersonas mp INNER JOIN mastempleado me ON (mp.CodPersona=me.CodPersona) INNER JOIN mastdependencias md ON (me.CodDependencia=md.CodDependencia) INNER JOIN seguridad_alterna sa ON (me.CodDependencia=sa.CodDependencia AND sa.CodAplicacion='".$_SESSION["APLICACION_ACTUAL"]."' AND sa.Usuario='".$_SESSION["USUARIO_ACTUAL"]."' AND sa.FlagMostrar='S') LEFT JOIN rh_motivocese rmc ON (me.CodMotivoCes=rmc.CodMotivoCes) WHERE (mp.EsEmpleado='S') $filtrado ".$ordenar." LIMIT ".$limit.", $MAXLIMIT";
					$query=mysql_query($sql) or die ($sql.mysql_error());
				...
				<script type='text/javascript' language='javascript'>
					totalRegistrosP($registros, \"$_ADMIN\", \"$_INSERT\", \"$_UPDATE\", \"$_DELETE\");
