...
<tr>
 ...
 <td width="634">
   <select name="organismo" id="organismo" >
	...
	?></select>
   <input type="hidden" name="codBeneficio" id="codBeneficio" value="<?=$coBeneficio;?>" /></td>
...
<tr>
 ...
 <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblLista">
   ...
   <tr>
     <td><input type="checkbox" name="ch_planilla" id="ch_planilla"  /></td>
     <td>Planilla de Solicitud</td>
     <td>&nbsp;</td>
   </tr>
   <tr>
     <td width="8%"><input type="checkbox" name="ch_informe" id="ch_informe" /></td>
     <td width="66%">Informe Médico</td>
     <td width="26%">&nbsp;</td>
   </tr>
   <tr>
     <td><input type="checkbox" name="ch_factura" id="ch_factura" /></td>
     <td>Facturas</td>
     <td>&nbsp;</td>
   </tr>
   <tr>
     <td><input type="checkbox" name="ch_recipe" id="ch_recipe" /></td>
     <td>Recipe Médico</td>
     <td>&nbsp;</td>
   </tr>
   <tr>
     <td><input type="checkbox" name="ch_otro" id="ch_otro"  /></td>
     <td>Otro</td>
     <td>&nbsp;</td>
   </tr>
   <tr>
   ...
   <tr>
     <td>&nbsp;</td>
     <td>Disponible</td>
     <td id="td_disponible" align="right" style="font-weight:bold">0,00</td>
   </tr>
   <tr>
     <td>&nbsp;</td>
     <td>Consumido</td>
...
<tr>
 ...
 <td colspan="2">
 <select name="sel_tipo_solicitud" id="sel_tipo_solicitud" >
...
<tr>
 ...
 <select name="sel_servicio" id="sel_servicio" onchange="colocarResponsable(); calcularConsumo(value)" >
   </select>
   *</td>
 <td align="left">&nbsp;</td>
 </tr>
...
<tr>
 ...
 <td colspan="2"><select name="sel_rama" id="sel_rama" >
   </select>
   *</td>
 <td width="129">&nbsp;</td>
 </tr>
<tr>
  ...
  <td colspan="2"><select name="sel_institucion" id="sel_institucion" style='width:70%' >
    </select>
    *</td>
  <td>&nbsp;</td>
  </tr>
<tr>
  ...
  <td colspan="2"><select name="sel_medico" id="sel_medico" >
    </select>
    *</td>
  <td>&nbsp;</td>
  </tr>
...
<tr>
 ...
 <td width="661"><? $ano = date('Y'); // devuelve el año
   ...
   <input title="A&ntilde;o de Presupuesto" name="ejercicioPpto" type="text" value="" style="text-align:right" id="ejercicioPpto" size="3" maxlength="4" />* 
   F.Creaci&oacute;n:<input name="fcreacion" type="text" id="fcreacion" size="8" value="" readonly /> 
...
<!---/////////////////// ********   CARGAR SELECT SECTOR PRESUPUESTO  *******   /////////////////////-->
 <table width="800" class="tblForm" border="0px">
    ...
    <tr>
	  <td>&nbsp;</td>
	  <td ><input name="txt_codigo" id="txt_codigo" type="text"   size="12" maxlength="*"   /></td>
	  <td><input name="txt_descripcion" id="txt_descripcion" type="text"  style="width:98%"  size="60" maxlength="*" /></td>          
	  <td ><input name="txt_monto_item" id="txt_monto_item" type="text" value="0,00" onChange=""  maxlength="*" style="text-align:right; width:98%;" onkeypress="return(formatoMoneda(this,'.',',',event));" 	onkeyup="agregarItemEnter(event)" /></td>
	  <td >&nbsp;</td>                       
	</tr>
...
<tr><td></td>
	<? $ahora=date("Y-m-d H:m:s");
	   echo"<td class='tagForm'>&Uacute;ltima Modif.:</td>
		   <td coslpan='1'>
			 <input name='ult_usuario' type='text' id='ult_usuario' size='30' readonly value='".$_SESSION['USUARIO_ACTUAL']."' />
			 <input name='ult_fecha' type='text' id='ult_fecha' size='22' readonly value='".$ahora."' />
		   </td>";
   ?>
</tr>
</table>
<center>
<input name="btguardar" type="submit" id="btguardar" value="Editar Registro" onclick="validarCampoBeneficio()"/>
<input name="btcancelar" type="button" id="btcancelar" value="Cancelar" onclick="location.href='beneficioListar.php'"/>
</center></div>
...
function fechaJStoMySql(fecha){
    ...
    function guardarBeneficio()
    {
		...
		var codigoBen		=	xGetElementById('codBeneficio').value;
    ...
    function respGuardarBeneficio(req)
    {
        ...
        if(req.responseText == '1')
        {
			...
			location.href='beneficioListar.php';
        }
        else
        {
...
function respCargarDatosBeneficio(req, cond)
{
	...
	xGetElementById('codempleado').value=datos[0]['codPersona'];
	...
	xGetElementById('fcreacion').value=fechaJStoMySql(datos[0]['fechaEjecucion']);
	...
	function calcularConsumo(codEspecifico)
{
	//alert(codigo);
	//var codigo = xGetElementById('codempleado').value;
	var opcion = 'CONSUMOESPECIFICO';
	var url = 'lib/transBeneficio.php';
	AjaxRequest.post
												(
								       				{
								       					'parameters':{'opcion':opcion, 																	 
																	  'codEspecifico':codEspecifico
																	  }
								            			,'url':url
								            			,'onSuccess': respCalcularConsumo												       
														,'onError':function(req)
														{ 
								                        	alert('Error!\nStatusText='+req.statusText+'\nContents='+req.responseText);
								                        }         
...
function respCalcularConsumo(req)
{
	//alert('dfdsf');
	//var consumido  = '';
	var disponibleTotal = '';
	//var disponibleEsp ='';
	var resp =eval ("("+req.responseText+")");
	//alert(resp[0]['limiteAyudaE']+' '+resp[0]['limiteAyudaG']+' '+resp[0]['CONSUMIDO'])
	//alert(resp[0]['CONSUMIDO']);
	/*if(resp[0]['CONSUMIDO']==null)
		consumido  = '0,00';
	else{*/
		//consumido = toCurrency(resp[0]['CONSUMIDO']);
		var consumido = xGetElementById('td_consumido').innerHTML;
			consumido =quitarMiles(consumido);
		//xGetElementById('hid_limite_servicio').value=resp[0]['limiteAyudaE'];
			//alert(resp[0]['limiteAyudaG']+'-'+consumido.replace(',','.'));
		disponibleTotal = resp[0]['limiteAyudaG']-consumido.replace(',','.');
		
		//alert(disponibleTotal);
		disponibleTotal=toCurrency(disponibleTotal);						
		
	/*}*/
	//xGetElementById('td_consumido_serv').innerHTML=consumido;
	xGetElementById('td_disponible').innerHTML=disponibleTotal;
	//alert(req.responseText);
	consumidoPorPersonaEspecifico(resp[0]['limiteAyudaE']);
	
}

function consumidoPorPersonaEspecifico(limite)
{
	var codigo = xGetElementById('codempleado').value;
	var codEspecifico = xGetElementById('sel_servicio').value;
	var opcion = 'CONSUMOXPERSONAESPECIFICO';
	var url = 'lib/transBeneficio.php';
	AjaxRequest.post
												(
								       				{
								            			...
								            			,'url':url
								            			,'onSuccess': function (req){respConsumidoPorPersonaEspecifico(req,limite)}
														,'onError':function(req)
														{ 
								                        	alert('Error!\nStatusText='+req.statusText+'\nContents='+req.responseText);
								                        }         
...
function respConsumidoPorPersonaEspecifico(req,limite)
{
	...
	var resp =eval ("("+req.responseText+")");
	//alert(resp[0]['limiteAyudaE']+' '+resp[0]['limiteAyudaG']+' '+resp[0]['CONSUMIDO'])
	//alert(resp[0]['CONSUMIDO']+'-'+limite);
	//alert(resp);
	if(resp==null){
		consumido  = '0.00';}
	else {
		consumido=resp[0]['CONSUMIDO'];
	...
	//alert(consumido+'-'+limite);
	disponibleEsp = limite-consumido;				
	consumido = toCurrency(consumido);
	
	xGetElementById('td_consumido_serv').innerHTML=consumido;
	xGetElementById('td_disponible_serv').innerHTML=toCurrency(disponibleEsp);
	//alert(req.responseText);
...
function respPreparadoPor(req)
{
	...
	echo 'cargarDatosBeneficio('.$coBeneficio.',"EDITAR");';
	echo '</script>';
	?>
</body>
