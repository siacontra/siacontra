...
<?php echo"<form name='frmentrada' action='contratos_vencidos.php?filtro=".$_GET['filtro']."' method='POST'>"; ?>
...
<table width="1000" class="tblBotones">
	<tr>
		...
		<td width="500" align="right">
			<input name="btNuevo" type="button" id="btNuevo" class="btLista" value="Nuevo" disabled onclick="cargarOpcion(this.form, 'contratos_renovar.php', 'BLANK', 'height=450, width=800, left=100, top=100, resizable=no');" />
...
if ($registros!=0) {
	//	CONSULTO LA TABLA
$sql="SELECT rc.CodPersona, mp.NomCompleto, rtc.Descripcion AS TipoContrato, me.Fingreso, me.CodEmpleado, rp.DescripCargo, mo.Organismo, rc.FechaDesde, rc.FechaHasta, rc.FlagFirma, rc.FechaFirma, DATEDIFF(rc.FechaHasta, NOW()) AS Faltan FROM rh_contratos rc INNER JOIN mastpersonas mp ON (rc.CodPersona=mp.CodPersona) LEFT JOIN rh_tipocontrato rtc ON (rc.TipoContrato=rtc.TipoContrato) INNER JOIN mastempleado me ON (rc.CodPersona=me.CodPersona) INNER JOIN mastorganismos mo ON (me.CodOrganismo=mo.CodOrganismo) INNER JOIN rh_puestos rp ON (me.CodCargo=rp.CodCargo) INNER JOIN seguridad_alterna sa ON (me.CodDependencia=sa.CodDependencia AND sa.CodAplicacion='".$_SESSION["APLICACION_ACTUAL"]."' AND sa.Usuario='".$_SESSION["USUARIO_ACTUAL"]."' AND sa.FlagMostrar='S') WHERE rc.Estado='VE' $filtrado ORDER BY rc.CodPersona LIMIT ".$_GET['limit'].", $MAXLIMIT";
	$query=mysql_query($sql) or die ($sql.mysql_error());
	...
	<table width='1000' class='tblLista'>
	  <tr class='trListaHead'>
			...
			<th width='250' scope='col'>Nombre Completo</th>
...
<script type='text/javascript' language='javascript'>
	totalContratos(2, $registros, \"$_ADMIN\", \"$_INSERT\", \"$_UPDATE\", \"$_DELETE\");
