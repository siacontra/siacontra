...
function bono_periodos_registrar_eventos_procesar(form) {
	...
	for(var i=0; n=frm.elements[i]; i++) {
		...
		else if  (n.name == "laborados_Fecha") {
			
		
			var Fecha = n.value.trim();
			if (!valFecha(Fecha) || Fecha.trim() == "") { error = "Se encontraron lineas con Fechas Incorrectas"; break; }
			else detalles_laborados += formatFechaAMD(Fecha) + ";char:td;";
		}else if (n.name == "laborados_Observaciones") detalles_laborados += changeUrl(n.value) + ";char:tr;";
	
	}
	var len = detalles_eventos.length; len-=9;
	detalles_eventos = detalles_eventos.substr(0, len);
	
	var len = detalles_laborados.length; len-=9;//
	detalles_laborados = detalles_laborados.substr(0, len);//
	...
	if (error != "") {
		...
		$.ajax({
			type: "POST",
			url: "lib/form_ajax.php",
			data: "modulo=bono_periodos_registrar_eventos_procesar&detalles_eventos="+detalles_eventos+"&detalles_laborados="+detalles_laborados+"&"+post,
			async: false,
			success: function(resp) {
				if (resp.trim() != "") cajaModal(resp, "error", 400);
				else form.submit();
