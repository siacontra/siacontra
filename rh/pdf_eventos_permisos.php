...
require('mc_table_permisos2.php');
require('fphp.php');
connect();
//---------------------------------------------------
$filtro=strtr($filtro, "*", "'");
...
elseif ($ad == $ah)
$periodo = "Del ".$dd." de ".getNombreMes($mesd)." Al ".$dh." De ".getNombreMes($mesh)." del  ".$ad;
else $periodo = "Del ".$dd." de ".getNombreMes($mesd)." del  ".$ad." Al ".$dh." De ".getNombreMes($mesh)." del  ".$ah;
...
while ($field_empleado = mysql_fetch_array($query_empleado)) {
	...
	$sql = "SELECT 
			...
			FROM
				rh_permisos rp
				INNER JOIN mastmiscelaneosdet mmd1 ON (rp.TipoFalta = mmd1.CodDetalle AND mmd1.CodMaestro = 'TIPOFALTAS' and mmd1.CodDetalle='05')
				INNER JOIN mastmiscelaneosdet mmd2 ON (rp.TipoPermiso = mmd2.CodDetalle AND mmd2.CodMaestro = 'PERMISOS' and  mmd2.CodDetalle='23')
			WHERE
	...
	if (mysql_num_rows($query_eventos) != 0) {
		...
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(10, 4);
		$pdf->Cell(100, 4, utf8_decode('Observación / Motivo'), 1, 0, 'C');
		//$pdf->Cell(60, 4, 'Tipo Permiso', 1, 0, 'C');
		$pdf->Cell(35, 4, 'Fecha', 1, 0, 'C');
		$pdf->Cell(20, 4, 'Hora', 1, 0, 'C');
		$pdf->Cell(15, 4, utf8_decode('Días H.'), 1, 0, 'C');
		$pdf->Cell(15, 4, utf8_decode('Días C.'), 1, 1, 'C');
		while ($field_eventos = mysql_fetch_array($query_eventos)) {
			...
			$pdf->Cell(15, 4,  ($dias_diferencia), 0, 1, 'C');
                        $sum_diasc += $dias_diferencia ;
			$pdf->Ln(2);
		...
		$pdf->Cell(15, 6, $sum_dias, 0, 0, 'C');
                $pdf->Cell(15, 6, $sum_diasc, 0, 0, 'C');
		//$pdf->Cell(10, 6, $sum_horas.':'.$sum_minutos, 0, 1, 'R');
		$sum_dias = 0;
                $sum_diasc = 0;
		$sum_horas = 0;
		$sum_minutos = 0;
