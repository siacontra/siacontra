...
function Cabecera($pdf) {
	...
	$pdf->Cell(190, 10, 'Consumo por Funcionario', 0, 1, 'C');	
	$pdf->SetDrawColor(255, 255, 255); $pdf->SetFillColor(255, 255, 255); $pdf->SetTextColor(0, 0, 0);
	$pdf->SetFont('Arial', 'B', 6);
	$pdf->SetWidths(array(15, 55, 45, 20, 20, 15, 20));
	$pdf->SetAligns(array('C', 'L', 'L', 'L', 'L', 'C','R'));
	$pdf->Row(array(utf8_decode('Cédula'), 'Nombre Completo', 'Familia', 'T. Solicitud', 'Fecha', 'Estado', 'Monto'));
...
while ($field=mysql_fetch_array($query)) {
	...
	$pdf->SetWidths(array(15, 55, 45, 20, 20, 15, 20));
	$pdf->SetAligns(array('C', 'L', 'L', 'L', 'L', 'C','R'));
	$pdf->Row(array($field['cedula'], utf8_decode($field['funcionario']), utf8_decode($field['cargafamiliar']), $solicituD, $field['fechasolicitud'], $est, number_format($field['montoTotal'],2,',','.')));
}
//---------------------------------------------------
