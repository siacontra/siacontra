...
elseif ($_POST['modulo']=="PAISES") {
	...
	if ($_POST['accion']=="GUARDAR") {		
		...
		if ($rows!=0)	$error="REGISTRO EXISTENTE";
			...
			$codigo=getCodigo("mastpaises", "CodPais", 5);
			//	INSERTO EL NUEVO REGISTRO
...
elseif ($_POST['modulo']=="SERIEOCUPACIONAL") {
	...
	if ($_POST['accion']=="GUARDAR") {		
		...
		if ($rows!=0)	$error="REGISTRO EXISTENTE";
			...
			//	INSERTO EL NUEVO REGISTRO
			$sql="INSERT INTO rh_serieocupacional VALUES ('$codigo', '".$_POST['grupo']."', '$descripcion', 'A', '".$_SESSION['USUARIO_ACTUAL']."', '$ahora')";
			$query=mysql_query($sql) or die ($sql.mysql_error());
		}
	}
	elseif ($_POST['accion']=="ACTUALIZAR") {
...
elseif ($_POST['modulo']=="CARGOS") {
	...
	elseif ($_POST['accion']=="CARGOEXPERIENCIA") {
		...
		if ($_POST['area'] == '')
		{
			$sql="SELECT * FROM rh_cargoexperiencia WHERE CodCargo='".$_POST['cargo']."' AND CargoExperiencia='".$_POST['cargo_experiencia']."'";
...
//	MAESTRO DE ASIGNACION GLOBAL
else if ($modulo == "ASIGNACIONGLOBAL") {
	connect();
	$error=0;
	$ahora=date("Y-m-d H:i:s");
	//	Guardar registro....
	if ($accion == "GUARDAR") {
			...
			$sql = "INSERT INTO rh_ayudamedicaglobal (numAyudaG, 
													  decripcionAyudaG, 
													  limiteAyudaG, 
													  UltimoUsuario, 
													  UltimaFecha) 
	...
	elseif ($accion == "ACTUALIZAR") {
		...
		$lim = str_replace('.','',$limite);
		$lim = str_replace(',','.',$lim);
			$sql = "UPDATE rh_ayudamedicaglobal SET decripcionAyudaG = '".($descripcion)."', 
												limiteAyudaG = '".$lim."',												
												UltimoUsuario = '".$_SESSION['USUARIO_ACTUAL']."', 
												UltimaFecha = '".date("Y-m-d H:i:s")."' 
											WHERE 
	...
	//	Eliminar registro....
	elseif ($accion == "ELIMINAR") {
		$sql = "DELETE FROM rh_ayudamedicaglobal WHERE codAyudaG = '".$codigo."' AND codAyudaG NOT IN (select codAyudaG from rh_ayudamedicaespecifica)";
		$query = mysql_query($sql) or die ($sql.mysql_error());
	}
	echo $error;
}
...
//	MAESTRO DE ASIGNACION GLOBAL
else if ($modulo == "ASIGNACIONESPECIFICO") {
	connect();
	$error=0;
	$ahora=date("Y-m-d H:i:s");
	//	Guardar registro....
	if ($accion == "GUARDAR") {
		$lim = str_replace('.','',$limite);
		$lim = str_replace(',','.',$lim);
			$sql = "INSERT INTO rh_ayudamedicaespecifica (numAyudaE, 
													  codAyudaG,
													  decripcionAyudaE, 
													  limiteAyudaE,
													  CodPerAprobar,
	...
	//	Modificar registro....
		...
		$lim = str_replace('.','',$limite);
		$lim = str_replace(',','.',$lim);
			$sql = "UPDATE rh_ayudamedicaespecifica SET decripcionAyudaE = '".($descripcion)."', 
												codAyudaG = '".$asigGlobal."',
												limiteAyudaE = '".$lim."',	
												CodPerAprobar ='".$personaAprobar."',										
												UltimoUsuario = '".$_SESSION['USUARIO_ACTUAL']."', 
												UltimaFecha = '".date("Y-m-d H:i:s")."' 
											WHERE 
												 	numAyudaE = '".$codigo."'";
	...
	//	Eliminar registro....
	elseif ($accion == "ELIMINAR") {
		$sql = "DELETE FROM rh_ayudamedicaespecifica WHERE codAyudaE = '".$codigo."'";
		$query = mysql_query($sql) or die ($sql.mysql_error());
	}
	echo $error;
}
...
//	BENEFICIO
	...
	//	Eliminar registro....
		...
		$sql = "DELETE FROM rh_item_beneficio WHERE codBeneficio = '".$codigo."'";
		$query = mysql_query($sql) or die ($sql.mysql_error());
		
		$sql2 = "DELETE FROM rh_beneficio WHERE codBeneficio = '".$codigo."'";		
		$query = mysql_query($sql2) or die ($sql2.mysql_error());
	}
	
	else if ($accion == "ANULAR") {
		$sql = "UPDATE rh_beneficio SET estadoBeneficio='AN' WHERE codBeneficio = '".$codigo."' AND estadoBeneficio <> 'AP'" ;
		$query = mysql_query($sql) or die ($sql.mysql_error());
		
		
	} else if ($accion == "ELIMINARUTILES") {
		$sql2 = "DELETE FROM rh_familarutilesbeneficio WHERE codbeneficiarioutiles = '".$codigo."'";		
		$query = mysql_query($sql2) or die ($sql2.mysql_error());
		
		$sql = "DELETE FROM rh_beneficiarioutiles WHERE codbeneficiarioutiles = '".$codigo."'";
		$query = mysql_query($sql) or die ($sql.mysql_error());
