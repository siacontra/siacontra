...
function firma()
{
	...
	$strConsulta = "SELECT * from automotor.mantenimiento where id_mant =  '".$id_mant."'";
    ...
    $this->Text(24, 240,$nombres1,0,1);
	$this->Text(24, 244,$cargo1,0,1);
	$this->SetFont('Arial','B',10);
	$this->text(160, 220, 'Recibido por: ');
	$this->SetFont('Arial','B',7);
	//$this->text(155, 239, 'Nombre y Apellido/Firma/Fecha: ');
    $this->Line(150,235,200,235);
	$this->Text(148, 240,$nombres2,0,1);
	$this->Text(148, 244,$cargo2,0,1);
...
function Footer()
{
	...
	$id_man= $_REQUEST['id_mant'];
	$id_mant= $id_mant-0;
	$con = new DB;
	$consejocomunal = $con->conectar();	
	$strConsulta = "SELECT * from automotor.mantenimiento where id_mant =  '".$_REQUEST['id_mant']."'";
	$consejocomunal = mysql_query($strConsulta);
	$fila = mysql_fetch_array($consejocomunal);
	...
	$pdf->Text(64,70,$fila_dep['Dependencia'],0,1);
	...
	$pdf->Text(175,95,$fila['modelo'],0,1);
	$pdf->Text(175,99,$fila['ano'],0,1);
	...
	$pdf->Text(56,183,$fila['especificar'],0,1);
	$pdf->Text(50,189,$fila['observaciones'],0,1);
	$pdf->Text(43,177,$fila['proveedor'],0,1);
	$pdf->SetFont('Arial','',7);
	//$pdf->Text(28, 239,$fila_p['NomCompleto'],0,1);
