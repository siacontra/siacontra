...
if ($_POST['filtro']!="") $sql="SELECT * FROM pf_dependenciasexternas WHERE (CodDependencia LIKE '%".$_POST['filtro']."%' OR Dependencia LIKE '%".$_POST['filtro']."%') and Estado='A'";
else $sql="SELECT * FROM pf_dependenciasexternas WHERE  Estado='A'";
$query=mysql_query($sql) or die ($sql.mysql_error());
$registros=mysql_num_rows($query);
//
?>
<form name="frmlista" id="frmlista" method="post" action="lista_dependenciasext.php?limit=0&ventana=insertarDestinatarioDep&tabla=item">
	...
	if ($registros!=0) {
		...
		if ($_POST['filtro']!="") $sql="SELECT 
									    ...
									    WHERE 
										    ...
										    Representante LIKE '%".$_POST['filtro']."%' AND
											Estado='A'
									ORDER BY 
		...
		for ($i=0; $i<$rows; $i++) {
			...
			<tr class='trListaBody' onclick='mClk(this, \"registro\"); insertarDestinatarioDepExt(this.id,\"".$ventana."\");' id='a|".$field['CodDependencia']."'>
				...
				<td align='left'>".htmlentities($field['Representante'])."</td>
			</tr>";
