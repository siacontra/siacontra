...
if ($_POST['filtro']!="") $sql="SELECT * FROM pf_organismosexternos WHERE (CodOrganismo LIKE '%".$_POST['filtro']."%' OR Organismo LIKE '%".$_POST['filtro']."%') and Estado='A'";
else $sql="SELECT * FROM pf_organismosexternos WHERE Estado='A'";
	...
	if ($registros!=0) {
		...
		if ($_POST['filtro']!="") $sql="SELECT 
									    ...
									    WHERE 
											...
											DescripComp LIKE '%".$_POST['filtro']."%' AND
											Estado='A'
									ORDER BY 
		...
		for ($i=0; $i<$rows; $i++) {
			...
			<tr class='trListaBody' onclick='mClk(this, \"registro\"); insertarDestinatarioOrgExt(this.id,\"".$ventana."\");' id='".$field['CodOrganismo']."'>
				...
				<td align='left'>".htmlentities($field['Organismo'])."</td>
				<td align='left'>".htmlentities($field['RepresentLegal'])."</td>
			</tr>";
			}else{
			echo "
