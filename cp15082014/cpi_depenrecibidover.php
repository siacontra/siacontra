...
<body>
<? 
 $sa="select * from cp_documentodistribucion where Cod_Documento='".$_GET['registro']."' "; //echo $sql;
 $qa=mysql_query($sa) or die ($sa.mysql_error());
 $fa=mysql_fetch_array($qa);
 
 if($fa['Procedencia']=='EXT'){
   
   $sql="select * from cp_documentoextentrada where Cod_Documento='".$fa['registro']."' "; //echo $sql;
   $qry=mysql_query($sql) or die ($sql.mysql_error());
   $field=mysql_fetch_array($qry); 
?>
<!-- ***********************************************************  MOSTRAR DOCUMENTO EXTERNO *************************************************** -->
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Documentos Recibidos | Ver Registro</td>
...
<?
 ...
 $s="select * from cp_documentointerno where Cod_DocumentoCompleto='".$_GET['registro']."'";
...
<tr>
 ...
 <td width="325"><input type="text" id="fecha_documento" name="fecha_documento" size="10" value="<?=$f_Registro;?>" style="text-align:right" readonly/>*(dd-mm-aaaa)</td>
</tr>
<tr style="border-style:double">
 <td class="tagForm">Nro. Documento:</td>
...
<tr>
  <td class="tagForm">Descripci&oacute;n:</td>
  <td colspan="3"><textarea name="descrip" id="descrip" rows="2" cols="80" readonly><?=$f['Descripcion']?></textarea></td>
</tr>
<tr><td height="5"></td></tr>
<tr>
  ...
  <td valign="top" style="height:150px; width:800px;">
    ...
    <tr>
      ...
      <table width="800px" class="tblLista">
	   ...
	   $sdist="select * from cp_documentodistribucion where Cod_Documento='".$f['Cod_DocumentoCompleto']."' and Periodo ='".date("Y")."' and Procedencia = 'INT'";
