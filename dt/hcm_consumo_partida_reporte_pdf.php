...
while ($field2=mysql_fetch_array($query2)) {
	...
	$sql_d ="SELECT SUM(montoTotal) AS solicitado
	FROM rh_beneficio 
	WHERE codPersona = '".$registro."' 
	AND codAyudaE='".$field2['codAyudaE']."'
	AND anhoEjecucio='".date('Y')."'";
	$query_d=mysql_query($sql_d) or die ($sql_d.mysql_error());
	$field_d=mysql_fetch_array($query_d);
	...
	//$numSolicitud='';
	//$monto='';
	$disponible=$field2['limiteAyudaE']-$field_d['solicitado'];
	//$cedula='';
	//$fecha ='';
	...
	$pdf->Row(array('', '', utf8_decode($field2['decripcionAyudaE']),number_format($field2['limiteAyudaE'],2,',','.'), '', '', number_format($field_d['solicitado'],2,',','.'),number_format($disponible,2,',','.')));
...
$pdf->SetXY(20, $y+15); $pdf->Cell(190, 5,utf8_decode( '_________________________________'), 0, 1, 'L');
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY(20, $y+20); $pdf->MultiCell(190, 5, utf8_decode($field_firma["Nombres"].' '.$field_firma["Apellido1"].' '.$field_firma["Apellido2"]), 0, 'L');//$pdf->Cell(190, 5, utf8_decode('Nathalie Ordosgoite  ___________'), 0, 1, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->SetXY(20, $y+25); $pdf->MultiCell(190, 5, utf8_decode( $field_firma['DescripCargo']), 0, 'L');//$pdf->Cell(190, 5, utf8_decode('ASISTENTE ADMINISTRATIVO V GRADO 09 PASO D'), 0, 1, 'L');


//$y=$pdf->GetY($y+2);

$pdf->SetXY(120, $y); $pdf->Cell(190, 5,utf8_decode( 'Revisado por:'), 0, 1, 'L');
$pdf->SetXY(120, $y+15); $pdf->Cell(190, 5,utf8_decode( '_________________________________'), 0, 1, 'L');
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY(120, $y+20); $pdf->MultiCell(190, 5, utf8_decode('DRA. YUSMILY JOSEFINA CAMPOS GONZALEZ'), 0, 'L');//$pdf->Cell(190, 5, utf8_decode('Nathalie Ordosgoite  ___________'), 0, 1, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->SetXY(120, $y+25); $pdf->MultiCell(190, 5, utf8_decode('DIRECTORA DE RECURSOS HUMANOS'), 0, 'L');//$pdf->Cell(190, 5, utf8_decode('ASISTENTE ADMINISTRATIVO V GRADO 09 PASO D'), 0, 1, 'L');
