...
FROM
dt_servicio AS dts
INNER JOIN mastpersonas AS mpS ON dts.codSolicitante = mpS.CodPersona
INNER JOIN mastempleado AS mpF ON mpS.CodPersona = mpF.CodPersona
...
<?
echo"<input type='hidden' name='regresar' id='regresar' value='beneficioListar.php'/>";
echo "
 <table align='center'>
 <tr>
  <td>
...
<td align="right">
<input name="btNuevo" type="button" class="btLista" id="btNuevo" value="Nuevo" onclick="cargarPagina(this.form, 'servicioCrear.php');" />
<input name="btEditar" type="button" class="btLista" id="btEditar" value="Editar" onclick="cargarOpcion(this.form, 'beneficioEditar.php?accionE=EDITAR','SELF');"/>
<input name="btEliminar" type="button" class="btLista" id="btEliminar" value="Eliminar" onclick="eliminarBeneficio(this.form, 'beneficioListar.php?accion=ELIMINARBENEFICIO', '1', 'BENEFICIO');" />
<input name="btVer" type="button" class="btLista" id="btVer" value="Ver" onclick="cargarOpcion(this.form, 'beneficioVer.php?accion=VER', 'BLANK', 'height=500, width=950, left=200, top=200, resizable=no');" /> 
<input name="btPDF" type="button" class="btLista" id="btPDF" value="PDF"  onclick="cargarOpcion(this.form, 'beneficio_pdf.php', 'BLANK', 'height=800, width=800, left=200, top=200, resizable=yes');" />  
<input name="btAnular" type="button" class="btLista" id="btAnular" value="Anular" onclick="anularBeneficio(this.form,'beneficioListar.php');" />
</td>
</tr>
</table>
...
<table width="900" class="tblLista">
  <tr class="trListaHead">
	...
	<th width="110" scope="col">Cod Servicio</th>
    <th width="82" scope="col">Solicitante</th>
...
echo "<div  style='height:320px; overflow:scroll; overflow-x:hidden'>  
   <table width:'900px' class='tblLista'>";
	$j=1;
	  ...
	  // $field['tx_estatus'];
	   if($field['codStatus']=='PE'){$est='Pendiente';}
	   if($field['codStatus']=='RV'){$est='Revisado';}
	   if($field['codStatus']=='AP'){$est='Aprobado';}
	   if($field['codStatus']=='GE'){$est='Generado';}
	   if($field['codStatus']=='AN'){$est='Anulado';}
      ...
      <tr class='trListaBody' onclick='mClk(this,\"registro\");' onmouseover='mOvr(this);' onmouseout='mOut(this);' id='".$field['codServicio']."'>
	  <td align='center' width='16'>".$fila."</td>
	  <td align='center' width='110'>".$field['codServicio']."</td>
