...
function selFuncionario(codigo,nombre,cargo,dependencia) {
	
	
	
var codpersona = codigo;
var nombreCompleto = nombre;
var tx_cargo = cargo;
var tx_dependencia = dependencia;
	//var codpersona = document.getElementById("codpersona").value;
//var registro = document.getElementById("registro").value;
//var periodo = opener.document.frmentrada.pdesde.value;
//var nomina = opener.document.frmentrada.nomina.value;
...
//	ajax.onreadystatechange=function() {
	...
	//		var resp = ajax.responseText.trim();
//var dato = resp.split("|");
			//opener.document.frmentrada.monto.value="11";
...
//*******************************************************************//
	 tabla=opener.document.getElementById('dataTable2');
        
         fila = tabla.insertRow(tabla.rows.length);
         
         
         fila.id =tabla.rows.length;  // Here you can set tr's id
       ...
       for(var i=0; i<colCount; i++) {
         ...
         //4 dependencia
          ...
          switch(i) {
				case 0:
				//celda.childNodes[0].value = "PERSONA";

                //celda.id="td_id" ; // Here you can set td's id
				//celda.name="nombre";  // Here you can set td's id
				//celda.className="nombre";  // Here you can set td's id
				//celda.value="valor" ; // Here you can set td's id
				
				celda.innerHTML= "<input type='checkbox' name='chk' />";
                 
				break;
				case 1:
				
				celda.innerHTML= "<input type='hidden' name='co_persona' width='10' value='"+codpersona+"'/>"+codpersona;
                 
				break;
				
				case 2:
				
				//celda.innerHTML= "<input type='text' name='tx_nombre' value='"+nombreCompleto+"' />";
                 celda.innerHTML= nombreCompleto;
				break;
				case 3:
				
				
				//celda.innerHTML= "<input type='text' name='tx_cargo' value='"+tx_cargo+"' />";
				  celda.innerHTML= tx_cargo;
                 
				break;
				case 4:
				
				
				//celda.innerHTML= "<input type='text' name='tx_dependencia' value='"+tx_dependencia+"' />";
				  celda.innerHTML= tx_dependencia;
         ...
         // FALTA VALIDAR QUE NO SE REPITA
           ...
           /*    switch(celda.childNodes[0].name) {
				case "codPersona":
					celda.childNodes[0].value = "PERSONA";
					break;
				case "tx_nombre":
					celda.childNodes[0].value = "NOMBRE";
					break;
				case "tx_cargo":
					celda.childNodes[0].value = "CARGO";
					break;
				case "tx_dependencia":
					celda.childNodes[0].value = "DEPENDENCIA";
					break;       
...
////****************************************************************/
	  /*    var table = opener.document.getElementById('dataTable2');
           ...
           //  console.log (newcell);
				...
				case "codPersona":
					newcell.childNodes[0].value = "PERSONA";
					break;
				case "tx_nombre":
					newcell.childNodes[0].value = "NOMBRE";
					break;
				case "tx_cargo":
					newcell.childNodes[0].value = "CARGO";
					break;
				case "tx_dependencia":
					newcell.childNodes[0].value = "DEPENDENCIA";
					break;       
 ...
 *  var table = document.getElementById(tableID);
            ...
            for(var i=0; i<rowCount; i++) {
                var row = table.rows[i];
                var codPersona = row.cells[1].childNodes[0];
                  if (null != codPersona ) {
                     
                     console.log(codPersona.value);
                   }
...
//**********************************************************************/
 function AsignarAnalistas(tableID,CODASISTENCIA) {
	
	
	
	 alert('Mensaje '+CODASISTENCIA);
...
/////////////////////////////////////////////////////////////////
 var table = document.getElementById(tableID);
            ...
            for(var i=0; i<rowCount; i++) {
                ...
                var codPersona = row.cells[1].childNodes[0];
                  ...
                  if (null != codPersona.value || codPersona.value!=undefined  ) {
 ...
 //**********************************************************************//						
								...
								ajax.open("POST", "fphp_funciones_nomina.php", true);
								ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
								ajax.send("accion=ASIGNAR&_CODPERSONA="+codPersona.value+"&_CODASISTENCIA="+CODASISTENCIA);
...
/******************************************************/ 
 function AsignarFuncionario(tableID,CODASISTENCIA) {
...
/////////////////////////////////////////////////////////////////
 ...
 //**********************************************************************//						
								...
								ajax.open("POST", "fphp_funciones_nomina.php", true);
								ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
								ajax.send("accion=ASIGNAR-FUNCIONARIO&_CODPERSONA="+codPersona.value+"&_CODASISTENCIA="+CODASISTENCIA);
...
function validarDetalleConcepto(form) {
	...
	if (chkasignacion == "" && chkdeduccion == "") {
		...
		if (document.getElementById("pdf").checked) {
			window.open("pdf_detallado_conceptos.php?forganismo="+forganismo+"&ftiponom="+ftiponom+"&fperiodo="+fperiodo+"&ftproceso="+ftproceso+"&chkdeduccion="+chkdeduccion+"&chkasignacion="+chkasignacion, "", "toolbar=no, menubar=no, location=no, scrollbars=yes, height=800, width=800, left=200, top=200, resizable=yes");
		} else {
			if (archivo == "") {
