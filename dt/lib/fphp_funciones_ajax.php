...
//	--------------------------
elseif ($accion == "bono_periodos_registrar_laborados_insertar") {
	list($AnioActual, $MesActual, $DiaActual) = split("[/.-]", substr($Ahora, 0, 10));
	$FechaActual = "$DiaActual-$MesActual-$AnioActual";
	?>
    <tr class="trListaBody" onclick="mClk(this, 'sel_laborados');" id="laborados_<?=$secuencia?>_<?=$nrodetalle?>">
    	<th>
			<?=$nrodetalle?>
        </th>
        ...
        <td>
            <select name="laborados_Fecha" id="Fecha_<?=$secuencia?>_<?=$nrodetalle?>" class="cell" onChange="getDiffHoraEventos('<?=$secuencia?>_<?=$nrodetalle?>');">
                <option value="">&nbsp;</option>
                <?=getFechaEventos($FechaInicio, $FechaFin, "", 0)?>
            </select>
        </td>
        ...
        <td>
            <textarea name="laborados_Observaciones" class="cell" style="height:15px;"></textarea>
        </td>
    </tr>
    <?
}
