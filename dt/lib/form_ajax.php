...
elseif ($modulo == "bono_periodos_registrar_eventos_procesar") {
	...
	//	elimino laborados
	$sql = "DELETE FROM rh_bonoalimentacionlaborados
			WHERE
				Anio = '".$Anio."' AND
				CodOrganismo = '".$CodOrganismo."' AND
				CodBonoAlim = '".$CodBonoAlim."' AND
...
$sql = "SELECT DiasPeriodo FROM rh_bonoalimentaciondet
			WHERE
				Anio = '".$Anio."' AND
				CodOrganismo = '".$CodOrganismo."' AND
				CodBonoAlim = '".$CodBonoAlim."' AND
...
$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
$field_datos_periodo = mysql_fetch_array($query);
$_dias_completos= $field_datos_periodo['DiasPeriodo'];
// actualizo todo los dias para el periodo  del empleado
			$_ValorPagar = 0;
			$_DiasInactivos = 0;
			$_dia_semana = getDiaSemana(formatFechaDMA($FechaInicio));
			$_fecha =formatFechaDMA($FechaInicio);
			//	obtengo la leyenda de los dias
			for ($i=1; $i<=$_dias_completos; $i++) {
				if ($_dia_semana == 7) $_dia_semana = 0;
				if ($_dia_semana >= 1 && $_dia_semana <= 5) {
...
if ($detalles_eventos != NUll || $detalles_eventos!="")
	...
	// dias_laborados	
	$_Secuencia = 0;
	$laborados = split(";char:tr;", $detalles_laborados);
if ($detalles_laborados != NUll || $detalles_laborados!="")
	foreach ($laborados as $linea) {
			list($_Fecha, $_Observaciones) = split(";char:td;", $linea);
			$_LaboradosFecha[$_Fecha] = $_Fecha;
		//	inserto
		$sql = "INSERT INTO rh_bonoalimentacionlaborados
				SET
					Anio = '".$Anio."',
					CodOrganismo = '".$CodOrganismo."',
					CodBonoAlim = '".$CodBonoAlim."',
     ...
     // sumo los dias laborados (extras) 
		$_DiasLaborados = 0;
	foreach ($_LaboradosFecha as $_Fecha) {
		
		list($_Horas, $_Minutos) = split("[:]", $_LaboradosFecha[$_Fecha]);
			$_DiasLaborados++;
