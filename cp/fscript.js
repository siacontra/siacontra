...
function totalRegistrosSalista(rows, admin, insert, update, del) {
	...
	if (!rows) btVer.disabled=true;
	if (del=="N") btEliminar.disabled=true; // Anular
	if (!rows) btImprimir.disabled=true; 
}
//// ------------------------------------------------------------------------------------------------------
...
function guardarSalidaExt(form) {  //alert('guardarSalidaExt');
	...
	for(i=0; n=frmdetalles.elements[i]; i++) {
		...
		if (n.name == "atencion") detalles += n.value + ";";
	} 
	var len = detalles.length; len--;
	detalles = detalles.substr(0, len);
...
function cargarDocumentoEnvioSalida(form) {
	...
	if (detalles == "") alert("¡Debe seleccionar por lo menos un detalle!");
	else window.open("cpe_salidadocumento.php?detalles="+detalles+"&formulario="+form.id, '_blank','height=580, width=850, left=200, top=70,scrollbars=yes'); 
}
//// ---------------------------------------------------------------------------
//// ---------------------------------------------------------------------------
