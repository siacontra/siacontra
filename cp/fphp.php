...
function getDependencias($dependencia, $organismo, $opt) {
	...
	switch ($opt) {
		case 0:
			...
			WHERE CodOrganismo='".$organismo."' 
			and  mastdependencias.Estado='A'
...
function getRepreExt($destinatario, $dependencia, $opt, $organismo) {
	...
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$destinatario) echo "<option value='".$field[0]."' selected>".htmlentities($field[0])."</option>"; 
...
connect();
	...
	case 0: 
			...
			for($i=0; $i<$rows; $i++){
			  ...
			  if($fcon[0]==$remitente_int) echo "<option value='".$fcon[0]."'>".htmlentities($fcon[1])."</option>";
			  else echo "<option value='".$fcon[0]."'>".htmlentities($fcon[1])."</option>";
			}
		   break;
    }
}
...
connect();
	...
	case 0: 
			...
			for($i=0; $i<$rows; $i++){
			  ...
			  $qcon=mysql_query($scon);
			  $fcon=mysql_fetch_array($qcon);
			  if($fcon[0]==$remitente_int) echo "<option value='".$fcon[0]."'>".htmlentities($fcon[0])."</option>";
			  else echo "<option value='".$fcon[0]."'>".htmlentities($fcon[0])."</option>";
			}
		   break;
    }
}
...
//// --------------------------------------------------------------------------------------
function getRemitenteDepRecibido($opt, $dependencia) {
connect();
 if ($opt==3 && $_SESSION["USUARIO_ACTUAL"]==$_SESSION["SUPER_USUARIO"]) $opt=0;
 switch ($opt) {
  case 0:
...
function getRemitente($opt, $remitente) {
	...
	switch ($opt) {
		case 0:			
			...
			for ($i=0; $i<$rorg; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$remitente) echo "<option value='".$field[0]."' selected>".htmlentities($field[1])."</option>"; 
				else echo "<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
			}
			break;
		case 1:
			...
			for($i=0; $i<$rows; $i++){
			   ...
			   if($field[0]==$remitente){
			     echo "<option value='".$field[0]."' selected>".htmlentities($field[0])."</option>";
				 $remit= $field[0];
			   } else{
			     echo "<option value='".$field[0]."'>".htmlentities($field[0])."</option>";
			   }
			}
			break;
	    case 2:
			...
			for($i=0; $i<$rorg; $i++){
			   ...
			   if ($forg[0]==$remitente) echo "<option value='".$forg[0]."' selected>".htmlentities($forg[1])."</option>"; 
			   else echo "<option value='".$forg[0]."'>".htmlentities($forg[1])."</option>"; 
			} 
			break;
		case 3:
			$sql="SELECT s.CodOrganismo, o.Organismo FROM seguridad_alterna s INNER JOIN mastorganismos o ON (s.CodOrganismo=o.CodOrganismo) WHERE s.Usuario='".$_SESSION["USUARIO_ACTUAL"]."' AND s.CodAplicacion='".$_SESSION["APLICACION_ACTUAL"]."' AND s.FlagMostrar='S' GROUP BY s.CodOrganismo ORDER BY s.CodOrganismo";
		...
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				...
				if ($field[0]==$recibidoPor) echo "<option value='".$field[0]."' selected>".$fpersona[0]."</option>"; 
					  ...
					  echo "<option value='".$field[0]."'>".htmlentities($fpersona[0])."</option>";
...
function getElaboradoPor($opt, $elaboradoPor) {
	...
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$elaboradoPor) echo "<option value='".$field[0]."' selected>".htmlentities($field[1])."</option>"; 
				else echo "<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
			}
			break;
	}
...
function getEstado($opt, $estado) {
	...
	switch ($opt) {
	   case 0:
		      for ($i=0; $i<$total; $i++) {
				if ($valor==$v_status[$i]) echo "<option value='".$v_status[$i]."' selected>".htmlentities($status[$i])."</option>";
				else echo "<option value='".$v_status[$i]."'>".htmlentities($status[$i])."</option>";
			  }
			  break;		
	   case 1:
		      for ($i=0; $i<$total; $i++) {
...
function getDestinatario($opt, $destinatario) {
	...
	switch ($opt) {
		...
		case 1:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$destinatario) echo "<option value='".$field[0]."' selected>".htmlentities($field[1])."</option>"; 
				else echo "<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
			}
			break;
		case 3:
...
connect();
   ...
   switch($opt){
      case 0:
            ...
            for($i=0; $i<$rows; $i++){
			  $field=mysql_fetch_array($qry);
			  if($field[0]==$tipodocumento) echo"<option value='".$field[0]."' selected>".htmlentities($field[1])."</option>";
			  else echo"<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
			}
			break;
	   case 1:
		...
		case 2:
            ...
            for($i=0; $i<$rows; $i++){
			  $field=mysql_fetch_array($qry);
			  if($field[0]==$tipodocumento) echo"<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
			  else echo"<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
			}
			break;
		case 3:
...
function getObtenerValorExterno($organismo, $dependencia){
	...
	$qry = mysql_query($sql) or die ($sql.mysql_error());
	if (mysql_num_rows($qry) != 0)  $field = mysql_fetch_array($qry);
	return array(utf8_encode($field[0]), ($field[1]));
}
//// -------------------------------------------------------------------------------------
//// -------------------------------------------------------------------------------------
function getProcedencia($opt, $procedencia) {
	...
	switch ($opt) {
		case 0:
		      for ($i=0; $i<$total; $i++) {
				if ($procedencia==$v_status[$i]) echo "<option value='".$v_status[$i]."' selected>".htmlentities($status[$i])."</option>";
				else echo "<option value='".$v_status[$i]."'>".htmlentities($status[$i])."</option>";
			  }
			  break;		
	}
...
function getDepDestinataria($opt, $dependencia) {
	...
	switch ($opt) {
		case 0:
			...
			AND CodDependencia = '".$fconsulta[1]."' 
			ORDER BY CodDependencia";
			$query=mysql_query($sql) or die ($sql.mysql_error());
			$rows=mysql_num_rows($query);
			for ($i=0; $i<$rows; $i++) {
		...
		case 1: 
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field['CodDependencia']==$dependencia) echo "<option value='".$field['CodDependencia']."' selected>".htmlentities($field['Dependencia'])."</option>"; 
				else echo "<option value='".$field['CodDependencia']."'>".htmlentities($field['Dependencia'])."</option>";
			}
		break;	
	}
}
...
connect();
   ...
   switch($opt){
      case 0:
            ...
            for($i=0; $i<$rows; $i++){
			  $field=mysql_fetch_array($qry);
			  if($field[0]==$particular) echo"<option value='".$field[0]."' selected>".htmlentities($field[1])."</option>";
			  else echo"<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
			}
			break;
   }
