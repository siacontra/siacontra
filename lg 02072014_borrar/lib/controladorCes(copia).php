...
//----------------------GUARDADO Y GENERACION DEL ACTA DE INICIO			
			case 'guardarGenerarActaInicio':
				...
				if($objConexion->ingresar(array("CodActaInicio,CodPersonaAsistente,CodPersonaDirector,AnioActa, FechaCreacion, NroVisualActaInicio, UltimoUsuario,UltimaFechaModif",
				"".$CodActaInicio.",'".$codAsistenteActaInicio."','".$codDirectorActaInicio."','".date('Y')."','".date('Y-m-d')."',".$numeroActa.",'".$_SESSION['CODPERSONA_ACTUAL']."',NOW()"),'lg_actainicio') == true)
				{
					$bandera = 1;
					...
					{
						/*$CodRequeSecue = array();
						$CodRequeSecue = explode('.', $vectorCodRequerimientoSecue[$i]);*/
						
						//print_r($CodRequeSecue);
			...
			case 'guardarEvaluacionProveedor':
				...
				if($objConexion->ingresar(array(" CodEvaluacion , CodActaInicio , ObjetoEvaluacion , CriterioCualitativo , CriterioCuantitativo , Conclusion , Recomendacion , CodPersonaAsistente , CodPersonaDirector,UltimoUsuario,UltimaFechaModif, AnioEvaluacion, NroVisualEvaluacion, FechaCreacion 
				...
				"','".$asistenteEvaluacion."','".$directorEvaluacion."','".$_SESSION['CODPERSONA_ACTUAL']."',NOW(),'".date('Y')."',".$numeroEvaluacion.",'".date('Y-m-d')."'"),'lg_evaluacion') == true)
				{
					$bandera = 1;
			...
			case 'modificarGenerarControlPerceptivo':
				...
				"".$CodControlPerceptivo.",'".$nroOrden."','".$persona[0]."','".$persona[1]."','".$persona[2]."','".$persona[3]."','".$persona[4]."','".date("Y-m-d")."','".$_SESSION['CODPERSONA_ACTUAL']."',NOW()"),'lg_controlperceptivo') == true)*/
				{
					...
					for($i = 0; $i < count ($resp2); $i++)
					{
						...
						if($objConexion->ingresar(array("CodControlPerceptivoDetalle,CodControlPerceptivo,CodItem,Secuencia,Recibido,ObservacionItem,CantidadRecibida,UltimoUsuario,UltimaFechaModif",
						"".$CodControlPerceptivoDetalle.",".$CodControlPerceptivo.",'".$resp2[$i]['CodItem']."',".$resp2[$i]['Secuencia'].",".$item[$i].",'".$observacionRecibido[$i]."',".$cantidadRecibido[$i].",'".$_SESSION['CODPERSONA_ACTUAL']."',NOW()"),'lg_controlperceptivodetalle') != true)
						{
							echo '0';
							break;
						}
			...
			case 'guardarModificarInformeRecomendacion':
				...
				if($codInformeRecomendacion == '0')
				{
					...
					if($objConexion->ingresar(array("CodInformeRecomendacion,CodEvaluacion,Asunto,ObjetoConsulta,Conclusiones,Recomendacion,Asistente,Director,
					UltimoUsuario, UltimaFechaModif, AnioRecomendacion, NroVisualRecomendacion, FechaCreacion,TipoAdjudicacion",
					"".$codInformeRecomendacion.",".$codEvaluacion.",'".$asunto."','".$objeto."','".$conclusion."','".$recomendacion."','".$persona[3]."','".$persona[4]."','".$_SESSION['CODPERSONA_ACTUAL']."',NOW(),'".date('Y')."',".$numeroRecomendacion.",'".date('Y-m-d')."','".valorListaTipoAdjudicacion."'"),'lg_informerecomendacion') == true)
					{
			...
			case 'guardarGenerarInformeAdjudicacion':
				...
				$resultado8 = $objConexion->consultar($sql8,'fila');
				$nroVisualAux = $resultado['NroVisualAdjudicacion'];
			...
			case 'generarEvaluacionActa':
				...
				$sql = "select * from lg_evaluacion where CodEvaluacion=".$codEvaluacion;
				$resp = $objConexion->consultar($sql,'fila');
