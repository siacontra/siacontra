...
//		$this->Ln(5);
		...
		$this->SetWidths(array(20, 18, 28, 18,55,15,15,15,21));
...
if ($fproveedor != "") $filtro2.=" AND (A.CodProveedor = '".$fproveedor."')";
...
//$pdf->SetXY(5, 0); $pdf->Cell(195, 5, utf8_decode('Recepciones por Proveedor'), 0, 1, 'L');			
			...
			{
				$sql1 = "select C.CodProveedor,A.CodDocumento,C.FechaAprobacion, C.NroOrden, B.Secuencia, A.Periodo, A.FechaDocumento, A.CodAlmacen, A.Comentarios, B.CodOrganismo, B.NroDocumento, B.Secuencia, B.CodItem, B.Descripcion, B.CodUnidad, B.CantidadRecibida, B.ReferenciaCodDocumento, B.ReferenciaNroDocumento, B.ReferenciaSecuencia, B.PrecioUnit 
						...
						where A.CodDocumento='NI' and  A.CodTransaccion='ROC'  and C.CodProveedor='".$resp[$i]['CodProveedor']."'
						$filtro1 $filtro";
...
//					$pdf->Ln(3);
				...
				for($j = 0; $j < count($resp1); $j++)
				{
					...
					$pdf->SetDrawColor(255, 255, 255);
					$pdf->SetFillColor(255, 255, 255);
					$pdf->SetWidths(array(20, 18, 28, 18,55,15,15,15,21));
