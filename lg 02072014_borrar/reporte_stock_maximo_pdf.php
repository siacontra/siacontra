...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetXY(5, 20); $this->Cell(195, 5, utf8_decode('Items con Stock Mayor al Stock Máxnimo'), 0, 1, 'C');
		$this->Ln(5);
		...
		//$this->SetXY(15, 25);
		$this->SetWidths(array(35, 67, 32, 32,32));
...
//			$pdf->SetX(15);
			...
			for($i = 0; $i < count($resp); $i++)
			{
				$sql1 = "SELECT A.CodItem, A.Descripcion, C.StockMin, C.StockMax, fami.Descripcion as DescripcionFami, line.Descripcion as DescripcionLine, (C.StockReorden-B.StockActual) as faltante,A.CodUnidad,C.StockReorden,B.StockActual
					...
					WHERE B.StockActual > C.StockMax and A.CodLinea='".$resp[$i]['CodLinea']."' and A.CodFamilia='".$resp[$i]['CodFamilia']."'
				...
				$pdf->Row(array(utf8_decode('Línea-Familia: '.$resp[$i]['DescripcionLine'].'-'.$resp[$i]['DescripcionFami'])));

				
				for($j = 0; $j < count($resp1); $j++)
				{
//					$pdf->SetX(15);
					$pdf->SetWidths(array(35, 67, 32, 32,32));
