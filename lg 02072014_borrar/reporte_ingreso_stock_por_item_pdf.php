...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetXY(5, 20); $this->Cell(195, 5, utf8_decode('Recepciones por Ítem/Proveedor'), 0, 1, 'C');
//		$this->Ln(5);
		...
		$this->SetWidths(array(20, 18, 28, 55,18,15,15,15,21));
...
//$pdf->SetXY(5, 0); $pdf->Cell(195, 5, utf8_decode('Recepciones por Proveedor'), 0, 1, 'L');			
			...
			$sql = "select distinct B.CodItem, B.Descripcion
					from lg_transacciondetalle as B 
					where 1";
			...
			{
				$sql1 = "select C.CodProveedor,C.NomProveedor,A.CodDocumento,C.FechaAprobacion, C.NroOrden, B.Secuencia, A.Periodo, A.FechaDocumento,
						A.CodAlmacen, A.Comentarios, B.CodOrganismo, B.NroDocumento, B.Secuencia, B.CodItem, B.Descripcion,
						B.CodUnidad, B.CantidadRecibida, B.ReferenciaCodDocumento, B.ReferenciaNroDocumento, B.ReferenciaSecuencia, B.PrecioUnit 
						from lg_transaccion as A 
						join lg_transacciondetalle as B on A.NroDocumento=B.NroDocumento 
						
						join lg_ordencompra as C on B.ReferenciaNroDocumento = C.NroOrden
						
						where A.CodDocumento='NI' and  A.CodTransaccion='ROC'  and B.CodItem='".$resp[$i]['CodItem']."'
				...
				if(count($resp1) > 0)
				{
					$pdf->Row(array($resp[$i]['CodItem'].' '.utf8_decode($resp[$i]['Descripcion'])));
//					$pdf->Ln(3);
				} 
				...
				for($j = 0; $j < count($resp1); $j++)
				{
					...
					$pdf->SetDrawColor(255, 255, 255);
					$pdf->SetFillColor(255, 255, 255);
					$pdf->SetWidths(array(20, 18, 28, 55,18,15,15,15,21));
...
/*					if($resp1[$j]['Descripcion'] == '')
					...
					$pdf->Row(array(utf8_decode($resp1[$j]['FechaDocumento']),
									 utf8_decode($resp1[$j]['Periodo']),
									 utf8_decode($resp1[$j]['CodDocumento']."-".$resp1[$j]['NroDocumento']),
									 utf8_decode($resp1[$j]['NomProveedor']),
									 $resp1[$j]['CodUnidad'],
									 $resp1[$j]['CantidadRecibida'],
									 $resp1[$j]['ReferenciaCodDocumento']."-".$resp1[$j]['NroOrden']."-".$resp1[$j]['Secuencia'],
									 $resp1[$j]['FechaDocumento'],
									 $resp1[$j]['CodAlmacen']));
