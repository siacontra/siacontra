...
var colorSeleccionTabla='#228dd0';
var colorFondoTabla='#e9e9e9';
var colorFilaCuentaAgregada='#B9C8FF';

function pintarFila(fila){
    colorBase=xGetElementById(fila).bgColor
    xGetElementById(fila).bgColor='#f5f5db';
}


function despintarFila(fila){
    xGetElementById(fila).bgColor=colorBase;
}
...
var IconoCargando="<IMG src='../img/cargando.gif' width='22' height='22' align='left' border='0'><DIV style=\"font-family : 'sans-serif', 'Arial','Bitstream Vera Sans'; font-size : 14px;\">Cargando...</DIV>";

var IconoCargandoTabla="<TR><TD align='center'><br><table><tbody><tr><td valign='middle'><IMG src='../img/cargando.gif' width='32' height='32' border='0'></td><td style=\"font-family : 'sans-serif', 'Arial','Bitstream Vera Sans'; font-size : 18px;\">Cargando...</td></tr></tbody></table></TD></TR>";
...
function ActivarBoton(_ID_BOTON,_ID_IMG,_ACCION){
    ...
    xGetElementById(_ID_IMG).setAttribute('src','img/iconos/'+_ACCION+'_activo.png');
    xGetElementById(_ID_BOTON).setAttribute('onmouseover',_ID_IMG+".src='img/iconos/"+_ACCION+"_con_foco.png'");
    xGetElementById(_ID_BOTON).setAttribute('onmouseout',_ID_IMG+".src='img/iconos/"+_ACCION+"_activo.png'");
}

function DesactivarBoton(_ID_BOTON,_ID_IMG,_ACCION){
    xGetElementById(_ID_IMG).setAttribute('src','img/iconos/'+_ACCION+'_desactivo.png');
    xGetElementById(_ID_BOTON).setAttribute('onmouseover',_ID_IMG+".src='img/iconos/"+_ACCION+"_desactivo.png'");
    xGetElementById(_ID_BOTON).setAttribute('onmouseout',_ID_IMG+".src='img/iconos/"+_ACCION+"_desactivo.png'");
    xGetElementById(_ID_BOTON).disabled=true;
}


function soloLet_S_N(evt)
{
    var nav4=window.Event?true:false;
    var key=nav4?evt.which:evt.keyCode;
	if(key==13) return false;//necesario para evitar recargas de página (ocurre ocacionalmente al presionar enter en el input text de una tabla. ejemplo presupuesto->clasificacion personal)

	

    return (key<=13 || key==127 || key==78 || key==83 || key==110 || key==115);
}
...
function CargarSELECT(req,IDSelect,IDSeleccionActual,NombreIdCampoTabla,NombreDenominacionCampoTabla,TamCodigo,TAM_MAX,NombreTitulo){
	if(!TamCodigo)
		TamCodigo=0;
...
function ContenidoSelectValue(SELECT){
	...
	function ContenidoSelectValueSelected1(SELECT){
	var S=xGetElementById(SELECT);
	//alert(S);
	var Arreglo=new Array();
