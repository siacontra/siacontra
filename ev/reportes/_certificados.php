<?php
	...
	$sql= "select tx_nombre_evento from ev_evento_capacitacion where co_id_evento=".$evento.";";
	...
	$pdf->Image("../img/CERTIFICADOS_11.jpg", 5, 15,270,187);
	//CERTIFICADOS_2.png
	...
	$pdf->SetFont('Arial','B',16);
		...
		$pdf->Ln(1);
		$pdf->MultiCell(0, 7, "C.I ".number_format($certificados[$i]["Ndocumento"],0,"","."), '', 'C', 0);
		...
		switch($mes2)
		{
			...
			if($mes1==$mes2)			
			$pdf->Cell($tam_ancho,4,"Cumaná, del ".$dia1." al ".$dia2." de ".$mes2." de ".$ano,'',1,'C',0);
			//$pdf->MultiCell(0, 7, "HHHH",'', 'C', 0);
			
			if($mes1!=$mes2)
			$pdf->Cell($tam_ancho,4,"Cumaná, del ".$dia1." de ".$mes1." al ".$dia2." de ".$mes2." de ".$ano,'',1,'C',0);
		...
		switch($mes)
		{
			...
			$pdf->Cell($tam_ancho,4,"Cumaná, ".$dia." de ".$mes." de ".$ano,'',1,'C',0);
		}
...
if($pon2[0]=="" || $pon2[0]==null)
{
	...
	$pdf->Cell($tam_ancho/2,4,"CONTRALOR PROVISIONAL DEL ESTADO SUCRE",'',0,'C',0);
...
else 
{
	...
	$pdf->Cell($tam_ancho/3,4,"CONTRALOR PROVISIONAL DEL ESTADO SUCRE",'',0,'C',0);
