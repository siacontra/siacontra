...
function setMontosOrdenCompra(frm_detalles) {
	...
	var MTotal = 0.00;//new Number(0.00);
	var ITotal = 0.00;
	for(var i=0; n=frm_detalles.elements[i]; i++) {
		if (n.name == "CantidadPedida") var CantidadPedida = new Number(setNumero(n.value));
			...
			if (FlagExonerado == "S") {
				...
				var Impuesto = new Number(setNumeroFormato(PrecioUnitTotal, 2, '', '.') * FactorImpuesto / 100);
				MontoAfecto += (setNumeroFormato(PrecioUnitTotal, 2, '', '.') * CantidadPedida);
			}
			Impuesto=new Number(setNumeroFormato(Impuesto, 2, '', '.'));
			PrecioUnitTotal = PrecioUnitTotal + Impuesto;
			n.value = setNumeroFormato(PrecioUnitTotal, 2, '.', ','); ITotal=new Number((ITotal+Impuesto)*CantidadPedida);
		}
		else if (n.name == "Total") {
			var Total = new Number(PrecioUnitTotal * CantidadPedida);
			n.value = setNumeroFormato(Total, 2, '.', ',');MTotal=new Number(MTotal+Total);
		} 
	}
	var MontoIGV = ITotal;//MontoAfecto * FactorImpuesto / 100;
	var MontoBruto = new Number(MontoAfecto + MontoNoAfecto);
	var MontoTotal = MTotal;//MontoBruto + MontoIGV;
