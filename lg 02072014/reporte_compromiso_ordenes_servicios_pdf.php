...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetXY(5, 20); $this->Cell(195, 5, utf8_decode('Ordenes de Servicios Distribución'), 0, 1, 'C');
		$this->Ln(5);
...
if ($fpreparaciond != "") $filtro.=" AND (os.FechaAprobacion >= '".formatFechaAMD($fpreparaciond)."')";
if ($fpreparacionh != "") $filtro.=" AND (os.FechaAprobacion <= '".formatFechaAMD($fpreparacionh)."')";
...
//				$pdf->Cell(235, 5,'Proveedor: '.$resp[$i]['CodProveedor'].' '.$resp[$i]['NomProveedor'].' Total proveedor: '.$resp[$i]['MontoTotal'], 0, 1, 'L');
			...
			for($i = 0; $i < count($resp); $i++)
			{
				$sql1 = "SELECT DISTINCT pa.denominacion, plc.Descripcion, dos.cod_partida, dos.CodCuenta, dos.Monto
				FROM lg_ordenservicio os
				INNER JOIN lg_distribucionos dos ON ( os.CodOrganismo = dos.CodOrganismo
				AND os.NroOrden = dos.NroOrden
				AND os.NroOrden = '".$resp[$i]['NroOrden']."' )
				JOIN pv_partida pa ON ( pa.cod_partida = dos.cod_partida )
				JOIN ac_mastplancuenta AS plc ON ( plc.CodCuenta = dos.CodCuenta )
				WHERE 1
				$filtro";
