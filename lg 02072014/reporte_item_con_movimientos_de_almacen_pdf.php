...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetXY(5, 20); $this->Cell(195, 5, utf8_decode('Ítems con movimientos de Almacén'), 0, 1, 'C', 0, 'reporteitem_sin_movimientos_de_almacen_pdf.php');
		$this->Ln(5);
...
$sql = "SELECT distinct
			...
			(SELECT SUM(td2.CantidadRecibida)
			 ...
			 WHERE
				...
				t2.FechaDocumento >= '".$fdesde."' AND
				t2.FechaDocumento <= '".$fhasta."' AND
		...
		FROM
			...
			LEFT JOIN lg_transaccion t ON (td.CodOrganismo = t.CodOrganismo AND
										   ...
										   t.FechaDocumento >= '".$fdesde."' AND
										   t.FechaDocumento <= '".$fhasta."')
		...
		WHERE i.CodItem in (SELECT CodItem FROM lg_requerimientosdet as A join  lg_requerimientos as B on B.CodRequerimiento=A.CodRequerimiento 
				where B.FechaRequerida <='".$fhasta."' and  B.FechaRequerida>='".$fdesde."')

		GROUP BY CodItem
		ORDER BY $forden";
//JOIN lg_requerimientosdet as rd on i.CodItem 		
//echo $sql;	exit;	

$query = mysql_query($sql) or die($sql.mysql_error());
while($field = mysql_fetch_array($query)) {
	$stock_inicial = $field['IngresosAnterior'] - $field['EgresosAnterior'];
	$stock_final = ($field['Ingresos'] - $field['Egresos']);
