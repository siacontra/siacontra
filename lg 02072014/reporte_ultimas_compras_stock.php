...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->SetXY(5, 20); $this->Cell(195, 5, utf8_decode('Últimas Compras Realizadas (Stock)'), 0, 1, 'C');
		$this->Ln(5);
		...
		$this->SetWidths(array(20, 15, 95, 20, 20, 15, 20));
...
$sql = "SELECT
		...
		FROM
			...
			INNER JOIN lg_itemmast i ON (ocd.CodItem = i.CodItem)
		WHERE 1 $filtro
		ORDER BY CodItem, CodProveedor";
...
while ($field = mysql_fetch_array($query)) {			$i++;
	...
	if ($grupo != $field['CodItem']) {
		...
		$pdf->Cell(20, 5, $field['CodItem'], 0, 0, 'C', 1);
		$pdf->Cell(10, 5, utf8_decode($field['CodUnidad']), 0, 0, 'C', 1);
		$pdf->Cell(150, 5, utf8_decode($field['Descripcion']), 0, 0, 'L', 1);
