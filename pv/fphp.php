...
function getFechaDias($fechad, $fechah) {
	...
	//convierto segundos en d\EDas
	$dias_diferencia = $segundos_diferencia / (60 * 60 * 24);
	
	//obtengo el valor absoulto de los d\EDas (quito el posible signo negativo)
	$dias_diferencia = abs($dias_diferencia);
	
	//quito los decimales a los d\EDas de diferencia
...
function getEstadoAjuste($status, $opt) {
	connect();
	$tstatus[0]="Pendiente"; $vstatus[0]="PR";
...
//	FUNCION PARA CARGAR LOS TIPOS DE STATUS EN UN SELECT PARA ESTADO ANTEPROYECTO
function getStatusAjuste($status, $opt) {
	connect();
	$tstatus[0]="Preparado"; $vstatus[0]="PE"; 
	$tstatus[1]="Revisado";  $vstatus[1]="RV";
	$tstatus[2]="Aprobado";  $vstatus[2]="AP";
...
function getTEvaluaciones2($valor, $opt) {
	connect();
	$tvalor[0]="90\BA"; $vvalor[0]="90";
	$tvalor[1]="180\BA"; $vvalor[1]="180";
	$tvalor[2]="360\BA"; $vvalor[2]="360";
	$total=3;
	switch ($opt) {
		case 0:
			for ($i=0; $i<$total; $i++) {
...
//	FUNCION PARA CARGAR LOS PREPARADOS EN UN SELECT PARA ANTEPROYECTO FILTRADO
function getCreditoPreparadoPor($fpreparado, $opt) {
	
	connect();
	switch ($opt) {
		case 0:
			$sql="SELECT `CodPersona` , `NomCompleto`
					FROM `mastpersonas`
					WHERE `CodPersona`
					IN (
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if($fpreparado==$field['CodPersona']){
				  echo "<option value='".$field['CodPersona']."' selected>".$field['NomCompleto']."</option>";
				}else{
				  echo "<option value='".$field['CodPersona']."'>".$field['NomCompleto']."</option>";
				}
			}
...
//	FUNCION PARA CARGAR LOS APROBADOS
function getCreditoAprobadoPor($fpreparado, $opt) {
...
if($accion=="ELIMINARCREDITOADICIONAL"){
	connect();
	$sql="DELETE FROM `pv_credito_adicional` WHERE `pv_credito_adicional`.`co_credito_adicional` = $codigo AND `pv_credito_adicional`.`tx_estatus` ='PE'"; 
	$query=mysql_query($sql) or die ($sql.mysql_error());
	
	//echo $query;
	if($query=='1')
	{
		$sql2="DELETE FROM `pv_item_credito_adicional`  WHERE `pv_item_credito_adicional`.`co_credito_adicional` = $codigo"; 	
		
		$query2=mysql_query($sql2) or die ($sql2.mysql_error());						
	}
}
if($accion=="ANULARCREDITO"){
	connect();
	$sql="UPDATE `pv_credito_adicional` SET `tx_estatus`='AN' WHERE `co_credito_adicional` = $codigo"; 	
	$query=mysql_query($sql) or die ($sql.mysql_error());
	
	
	 $sqlSel="SELECT b.cod_partida, 	b.mm_monto, a.CodAnteproyecto
			 FROM pv_credito_adicional as a, pv_item_credito_adicional as b
			 WHERE b.co_credito_adicional=$codigo AND 
			 		b.co_credito_adicional = a.co_credito_adicional and 
					a.tx_estatus<> 'AN'";
			 
	$querySel=mysql_query($sqlSel) or die ($sqlSel.mysql_error());
	//$field = mysql_fetch_array($querySel);
		
	
	if($querySel){
	
		while ($field = mysql_fetch_array($querySel, MYSQL_NUM))
		{
						...
						//$montoItem=str_replace(';', '', $montoItem);
						 ...
						 $sql_pre ="UPDATE pv_presupuestodet
									SET MontoAjustado = (MontoAjustado - $field[1] ) 
									WHERE
										
										CodPresupuesto = '".$field[2]."' AND
										cod_partida = '".$field[0]."'";
										$query=mysql_query($sql_pre) or die ($sql_pre.mysql_error());
...
if($accion=="REVISARCREDITO"){
	connect();
	$sql="UPDATE `pv_credito_adicional` SET `tx_estatus`='RV' WHERE `co_credito_adicional` = $codigo"; 	
	$query=mysql_query($sql) or die ($sql.mysql_error());	
}

if($accion=="APROBARCREDITO"){
	connect();
	$sql="UPDATE `pv_credito_adicional` SET `tx_estatus`='AP' WHERE `co_credito_adicional` = $codigo"; 	
	$query=mysql_query($sql) or die ($sql.mysql_error());	
}
