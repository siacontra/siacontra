 ...
 $field=mysql_fetch_array($query);
   if($cod_partida!=$field['cod_partida']){
	 if(($field[cod_tipocuenta]!=$valor) and ($field[partida1]==0)){ ?>
	  ...
	  if(($field[6]!=$valor) and ($field[partida1]!=0) and ($field[generica]==0)){ ?>
	 <tr class='trListaBody6'>
	   <td align="center"><?=$field['cod_partida']?></td>
	   <td><?=$field['denominacion']?></td>
	   <td align="center"><?=$field['tipo']?></td>
	   <td align="center"><input type="checkbox" name="$i" value=<?=$field['cod_partida']?> /></td>
	     ...
	     $valor=$field[cod_tipocuenta]; ?>
	 <tr class="trListaBody" onclick="mClk(this,'registro'); insertarItemRequerimiento(this.id,'<?=$ventana?>');" id="<?=$field['cod_partida']?>">
	  <td align="center"><?=$field['cod_partida']?></td>
	   <td><?=$field['denominacion']?></td>
	   <td align="center"><?=$field['tipo']?></td>
