...
<form id="frmentrada" name="frmentrada" action="presupuesto_nuevodetalle.php?limit=0&accion=GuardarMontoPartidas" method="POST" onsubmit="return verificarDet(this,'Guardar')">
...
<tr><td align="right">
  <input name="btNuevo" type="button" id="btNuevo" value="Agregar" onmouseup="Alarma(frmentrada)" onclick="cargarVentana(this.form,'lista_partidas.php?pagina=presupuesto_nuevodetalle.php?accion=AGREGAR&limit=0&registro=<?=$registro?>&ejercicioPpto=<?=$ejercicioPpto?>&target=main','height=500, width=800, left=200, top=200, resizable=yes');"/>
  <input name="btBorrar" type="button" id="btBorrar" value="Eliminar" onmouseup="Alarma2()" onClick="eliminarPartidaAnte(this.form,'presupuesto_nuevodetalle.php?accion=ELIMINAR&registro=<?=$registro?>');"/>
  </td>
</tr>
</table>
...
$sql="SELECT CodAnteproyecto FROM pv_antepresupuesto WHERE EjercicioPpto='".$_POST['ejercicioPpto']."' ";
$query=mysql_query($sql) or die ($sql.mysql_error());
...
<tr>
	...
	<td width="150"><input name="gaceta" id="gaceta" type="text" size="8" value="<?=$fieldAnt['FechaGaceta']?>" readonly/>*<i>(dd-mm-aaaa)</i></td>
...
<tr>
 ...
 <td width="150"><input name="fdecreto" id="fdecreto" type="text" size="8" value="<?=$fieldAnt['FechaDecreto']?>" readonly/>*<i>(dd-mm-aaaa)</i></td>
</tr>
<tr><td height="2"></td></tr>
</table>
<table width="800" class="tblForm">
