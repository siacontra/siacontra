...
function Header(){
	...
	$this->Cell(91, 3, '', 1, 0, 'C', 1);
	$this->Cell(24, 3, 'ASIGNACION', 1, 0, 'C', 1);
	$this->Cell(42, 3, 'MODIFICACIONES', 1, 0, 'C', 1);
	$this->Cell(21, 3, 'T. CREDITO', 1, 0, 'C', 1);
	$this->Cell(21, 3, 'ASIGNACION', 1, 0, 'C', 1);
	$this->Cell(63, 3, 'EJECUTADO', 1, 0, 'C', 1);
	$this->Cell(63, 3, 'PORCENTAJE', 1, 0, 'C', 1);
	$this->Cell(50, 3, 'DISPONIBILIDAD', 1, 0, 'C', 1);
	$this->Cell(50, 3, 'PORCENTAJE', 1, 1, 'C', 1);
	$this->SetFillColor(255, 255, 255);
	///// ******************	
	
	$this->SetDrawColor(0, 0, 0); $this->SetFillColor(200, 200, 200); $this->SetTextColor(0, 0, 0);
	$this->Cell(18, 3, 'PAR GE ESP SE', 1, 0, 'C', 1);
	$this->Cell(73, 3, 'DENOMINACION', 1, 0, 'C', 1);
	$this->Cell(24, 3, ' ORIGINAL', 1, 0, 'C', 1);
	$this->Cell(21, 3, 'AUNMENTO', 1, 0, 'C', 1);
	$this->Cell(21, 3, 'DISMINUCION', 1, 0, 'C', 1);
	$this->Cell(21, 3, ' ADICIONAL.', 1, 0, 'C', 1);
	$this->Cell(21, 3, ' AJUSTADA', 1, 0, 'C', 1);
	$this->Cell(21, 3, 'COMPROMISO', 1, 0, 'C', 1);
	$this->Cell(21, 3, 'CAUSADO', 1, 0, 'C', 1);
	$this->Cell(21, 3, 'PAGADO', 1, 0, 'C', 1);
	$this->Cell(21, 3, '% COMPROMISO', 1, 0, 'C', 1);
	$this->Cell(21, 3, '% CAUSADO', 1, 0, 'C', 1);
	$this->Cell(21, 3, '% PAGADO', 1, 0, 'C', 1);
	$this->Cell(25, 3, 'PRESUPUESTARIA', 1, 0, 'C', 1);
	$this->Cell(25, 3, 'FINANCIERA', 1, 0, 'C', 1);
	$this->Cell(25, 3, '% PRESUPUESTARIA', 1, 0, 'C', 1);
	$this->Cell(25, 3, '% FINANCIERA', 1, 1, 'C', 1);
	$this->SetFillColor(255, 255, 255);
	///// ******************	
}
//Page footer
...
$pdf=new PDF('L','mm',array('213','445'));
$pdf->AliasNbPages();
$pdf->AddPage();
 ...
 if(($fieldet['partida']!=00) and (($cont1==0) or ($pCapturada!=$fieldet['partida']))){
  ...
  if($rwPar!=0){
   ...
   /// - Consulta de partida incrementada o con ajuste positivo
	...
	/// ************************************************************
	/// ************************************************************
	 $ConIn0111 = "SELECT sum( mm_monto ) AS CreditoAparte
                        FROM pv_item_credito_adicional cadet
                        INNER JOIN pv_credito_adicional ca ON cadet.co_credito_adicional = ca.co_credito_adicional
                        WHERE ca.tx_estatus = 'AP'
                        AND cadet.cod_partida LIKE '".substr($fieldet['cod_partida'], 0, 3)."%'";	//echo $s_consulta03;				   
						   
	  $QueConIn0111 = mysql_query($ConIn0111) or die ($ConIn0111.mysql_error());
	  $ReConIn0111 = mysql_fetch_array($QueConIn0111);
	/// ************************************************************
	 $ConDi01 = "select 
	...
	/// ************************************************************
          ...
          if(!isset($ReConIn0111['CreditoAparte'])){
              $CreCre+=0;
          }else{
              $CreCre+=$ReConIn0111['CreditoAparte'];
          }
          ...
          $AjustePresupuestario=$InCre+$montoAprobado-$DiCre+$CreCre;
          $DisponibleFinaciero=$AjustePresupuestario-$montoPagado;
          $DisponiblePresupuestaria=$AjustePresupuestario-$montoCompromiso;
          
           if($DisponibleFinaciero<0){
             $DisponibleFinaciero=$DisponibleFinaciero*-1;
         }
         if($DisponiblePresupuestaria<0){
             $DisponiblePresupuestaria=$DisponiblePresupuestaria*-1;
         }
          $porcentajeCompromiso=  round(($montoCompromiso/$AjustePresupuestario)*100);
          $porcentajeCausado= round(($montoCausado/$AjustePresupuestario)*100);
          $porcentajePagado=  round( ($montoPagado/$AjustePresupuestario)*100);
          $porcentajeDisponibleFinaciero=  round( ($DisponibleFinaciero/$AjustePresupuestario)*100);
          $porcentajeDisponiblePresupuestaria=  round( ($DisponiblePresupuestaria/$AjustePresupuestario)*100);
	...
	$montoAjustado = number_format($AjustePresupuestario,2,',','.');
	 ...
	 $montoDisponibleP = number_format($DisponiblePresupuestaria,2,',','.');
	 $montoDisponibleF = number_format($DisponibleFinaciero,2,',','.');
	...
	$pdf->SetWidths(array(18, 73,24,21,21,21,21,21,21,21,21,21,21,25,25,25,25));
	$pdf->SetAligns(array('C','L','R','R','R','R','R','R','R','R','C','C','C','R','R','C','C'));
	$pdf->Row(array($codigo_partida,$fpar['denominacion'],$montoAprobado,$IntTotal1,$DitTotal1,$CretTotal1,$montoAjustado,$montoCompromiso,$montoCausado,$montoPagado,$porcentajeCompromiso.' %',$porcentajeCausado.' %',$porcentajePagado.' %',$montoDisponibleP,$montoDisponibleF,$porcentajeDisponiblePresupuestaria.' %',$porcentajeDisponibleFinaciero.' %'));
  }
 }
//// --------------------------------------------------------------------------------------------------
 ...
 if(($fieldet['partida']!=00) and (($cont2==0) or ($gCapturada!=$fieldet['generica']) or ($pCapturada2!=$fieldet['partida']))){
	...
	if($rows2!=0){
	  ...
	  $r_consulta02 = mysql_num_rows($q_consulta02);
          ...
          /// ************************************************************
	 $ConIn0112 = "SELECT sum( mm_monto ) AS CreditoAparte
                        FROM pv_item_credito_adicional cadet
                        INNER JOIN pv_credito_adicional ca ON cadet.co_credito_adicional = ca.co_credito_adicional
                        WHERE ca.tx_estatus = 'AP'
                        AND cadet.cod_partida LIKE '".substr($fieldet['cod_partida'], 0, 6)."%'";	//echo $s_consulta03;				   
						   
	  $QueConIn0112 = mysql_query($ConIn0112) or die ($ConIn0112.mysql_error());
	  $ReConIn0112 = mysql_fetch_array($QueConIn0112);
	  ...
	  if(!isset($ReConIn0112['CreditoAparte'])){
              $CreCre2=0;
          }else{
              $CreCre2=$ReConIn0112['CreditoAparte'];
          }
	  
          $montoAjustado1= $montoAprobado1+$montoIncremento2-$montoDisminucion2+ $montoCredito2+$CreCre2;
          $montoDisponibleP1 = $montoAjustado1 - $montoCompromiso1;
          $montoDisponibleF1 = $montoAjustado1 - $montoPagado1;
          
         if($montoDisponibleP1<0){
             $montoDisponibleP1=$montoDisponibleP1*-1;
         }
         if($montoDisponibleF1<0){
             $montoDisponibleF1=$montoDisponibleF1*-1;
         }
          
          $porcentajeCompromiso2=  round(($montoCompromiso1/$montoAjustado1)*100);
          $porcentajeCausado2= round(($montoCausado1/$montoAjustado1)*100);
          $porcentajePagado2=  round( ($montoPagado1/$montoAjustado1)*100);
          $porcentajeDisponibleFinaciero2=  round( ($montoDisponibleF1/$montoAjustado1)*100);
          $porcentajeDisponiblePresupuestaria2=  round( ($montoDisponibleP1/$montoAjustado1)*100);
                  
	   $montoAprobado1 = number_format($montoAprobado1,2,',','.');
	   $montoAjustado1 = number_format($montoAjustado1,2,',','.');	   
	   $montoCredito2 = number_format($montoCredito2+$CreCre2,2,',','.');	 
	   ...
	   $montoDisponibleP1 = number_format($montoDisponibleP1,2,',','.');
	   $montoDisponibleF1 = number_format($montoDisponibleF1,2,',','.');
	...
	$pdf->SetWidths(array(18, 73,24,21,21,21,21,21,21,21,21,21,21,25,25,25,25));
	$pdf->SetAligns(array('C','L','R','R','R','R','R','R','R','R','C','C','C','R','R','C','C'));
	  $pdf->Row(array($codigo_partida,$fpar2['denominacion'],$montoAprobado1,$montoIncremento2,$montoDisminucion2,$montoCredito2,$montoAjustado1,$montoCompromiso1,$montoCausado1,$montoPagado1,$porcentajeCompromiso2.' %',$porcentajeCausado2.' %',$porcentajePagado2.' %',$montoDisponibleP1,$montoDisponibleF1,$porcentajeDisponiblePresupuestaria2.' %',$porcentajeDisponibleFinaciero2.' %'));
   }
  }
//// --------------------------------------------------------------------------------------------------
...
//// --------------------------------------------------------------------------------------------------
 if($fieldet['partida']!=00){
	...
	/// ************************************************************
	 $ConIn0113 = "SELECT sum( mm_monto ) AS CreditoAparte
                        FROM pv_item_credito_adicional cadet
                        INNER JOIN pv_credito_adicional ca ON cadet.co_credito_adicional = ca.co_credito_adicional
                        WHERE ca.tx_estatus = 'AP'
                        AND cadet.cod_partida = '".$fieldet['cod_partida']."'";	//echo $s_consulta03;				   
						   
	  $QueConIn0113 = mysql_query($ConIn0113) or die ($ConIn0113.mysql_error());
	  $ReConIn0113 = mysql_fetch_array($QueConIn0113);
          
        $TotalIn=$montoIncremento3+$TotalIn;
        $TotalDi=$montoDisminucion3+$TotalDi;
        $TotalCre=$montoCredito3+$TotalCre+$ReConIn0113['CreditoAparte'];
	 
        $montoAjustado2=$fieldet['MontoAprobado']+$montoIncremento3-$montoDisminucion3+$montoCredito3+$ReConIn0113['CreditoAparte'];
          $montoDisponibleP2 = $montoAjustado2 - $fieldet['MontoCompromiso'];
          $montoDisponibleF2 = $montoAjustado2 - $fieldet['MontoPagado'];
          
          if($montoDisponibleP2<0){
             $montoDisponibleP2=$montoDisponibleP2*-1;
         }
         if($montoDisponibleF2<0){
             $montoDisponibleF2=$montoDisponibleF2*-1;
         }
          $porcentajeCompromiso2=  round(($fieldet['MontoCompromiso']/$montoAjustado2)*100);
          $porcentajeCausado2= round(($fieldet['MontoCausado']/$montoAjustado2)*100);
          $porcentajePagado2=  round( ($fieldet['MontoPagado']/$montoAjustado2)*100);
          $porcentajeDisponibleFinaciero3=  round( ($montoDisponibleF2/$montoAjustado2)*100);
          $porcentajeDisponiblePresupuestaria3=  round( ($montoDisponibleP2/$montoAjustado2)*100);
	 ...
	 $totalMontoAjustado =  $totalMontoAjustado + $montoAjustado2;
         
         
	 $montoAjustado2 = number_format($montoAjustado2 ,2,',','.');
	 ...
	 $montoDisponibleP2 = number_format($montoDisponibleP2,2,',','.');
	 $montoDisponibleF2 = number_format($montoDisponibleF2,2,',','.');
	 
	 
	 $montoCreditoA2 = number_format($montoCredito3 +$ReConIn0113['CreditoAparte'],2,',','.');
	 ...
	 $totalDisponibleP = ($totalMontoAjustado - $totalMontoCompromiso);
	 $totalDisponibleF = ($totalMontoAjustado - $totalMontoPagado);
	 
         
         if($totalDisponibleP<0){
             $totalDisponibleP=$totalDisponibleP*-1;
         }
         if($totalDisponibleF<0){
             $totalDisponibleF=$totalDisponibleF*-1;
         }
	...
	$pdf->SetWidths(array(18, 73,24,21,21,21,21,21,21,21,21,21,21,25,25,25,25));
	$pdf->SetAligns(array('C','L','R','R','R','R','R','R','R','R','C','C','C','R','R','C','C'));
	 $pdf->Row(array($fieldet['cod_partida'],$field['denominacion'],$montoAprobado2,$montoIncremento3,$montoDisminucion3,$montoCreditoA2,$montoAjustado2,$montoCompromiso2,$montoCausado2, $montoPagado2,$porcentajeCompromiso2.' %',$porcentajeCausado2.' %',$porcentajePagado2.' %', $montoDisponibleP2,$montoDisponibleF2,$porcentajeDisponiblePresupuestaria3.' %',$porcentajeDisponibleFinaciero3.' %' ));
 }
}
          
          $porcentajeCompromisoTotal=  round(($totalMontoCompromiso/$totalMontoAjustado)*100);
          $porcentajeCausadoTotal= round(($totalMontoCausado/$totalMontoAjustado)*100);
          $porcentajePagadoTotal=  round( ($totalMontoPagado/$totalMontoAjustado)*100);
          $porcentajeDisponibleFinacieroTotal=  round( ($totalDisponibleF/$totalMontoAjustado)*100);
          $porcentajeDisponiblePresupuestariaTotal=  round( ($totalDisponibleP/$totalMontoAjustado)*100);
	...
	///// *** Mostrar *** /////
	 ...
	 $totalDisponibleP = number_format($totalDisponibleP,2,',','.');
	 $totalDisponibleF = number_format($totalDisponibleF,2,',','.');
	
	$pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(202, 202, 202); $pdf->SetTextColor(0, 0, 0);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->SetWidths(array(18, 73,24,21,21,21,21,21,21,21,21,21,21,25,25,25,25));
	$pdf->SetAligns(array('C','L','R','R','R','R','R','R','R','R','C','C','C','R','R','C','C'));
	$pdf->Row(array('' ,'Total:',$totalMontoAprobado,$TotalIn,$TotalDi,$TotalCre,$totalMontoAjustado,$totalMontoCompromiso,$totalMontoCausado,$totalMontoPagado,$porcentajeCompromisoTotal.' %',$porcentajeCausadoTotal.' %',$porcentajePagadoTotal.' %',$totalDisponibleP,$totalDisponibleF,$porcentajeDisponiblePresupuestariaTotal.' %',$porcentajeDisponibleFinacieroTotal.' %'));
	/////
