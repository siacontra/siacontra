...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Aprobar Credito Adicional</td>
...
if ($fpreparado != "") { $filtro .= " AND (a.`tx_aprobado` = '".$fpreparado."')"; $cpreparado = "checked"; } else $dpreparado = "disabled";
...
//---------------------------------------------------------------------------------
echo "
<form name='frmentrada' action='creditoAdicionalAprobar.php?limite=0' method='POST'>";
...
echo "<td width='125' align='right'>Preparado Por:</td>
  <td><input type='checkbox' name='chkpreparado' value='1' $cpreparado $cstatus onclick='this.checked=true'' />
			<select name='fpreparado' id='fpreparado' class='selectBig' $dpreparado>
			    <option value='$fpreparado'>".$_SESSION['NOMBRE_USUARIO_ACTUAL']."</option>";
...
<input name="btAnular" type="button" class="btLista" id="btAnular" value="Aprobar" onclick="cargarOpcion(this.form, 'creditoAdicionalAprobarEditar.php?accion=APROBAR','SELF');"   /></td>
...
if($registros!=0){ 
	  ...
	  // $field['tx_estatus'];
	   ...
	   if($field['tx_estatus']=='RV'){$est='Revisado';}
	    if($field['tx_estatus']=='CF'){$est='Conformado';}
