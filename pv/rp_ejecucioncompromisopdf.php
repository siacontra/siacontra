...
function Header(){
	...
	$this->SetXY(20, 10); $this->Cell(120, 5,utf8_decode( 'Contraloría del estado Monagas'), 0, 0, 'L');
	                      $this->Cell(20,5,'Fecha:',0,0,'R');$this->Cell(15,5,date('d/m/Y'),0,1,'');
	$this->SetXY(20, 15); $this->Cell(120, 5, utf8_decode('Dirección de Administración y Presupuesto'), 0, 0, 'L');
	                       $this->Cell(20,5,utf8_decode('Página:'),0,1,'R');
	$this->SetXY(20, 20); $this->Cell(120,5,'', 0, 0, 'R');
	                       $this->Cell(20,5,utf8_decode('Año:'),0,0,'R');$this->Cell(15,5,date('Y'),0,1,'L');
	...
	$this->Cell(170, 10, utf8_decode('Ejecución Presupuestaria Compromisos '.$EjercicioPpto), 0, 1, 'C');
	//$this->Cell(80, 10, utf8_decode('Ejecución Presupuestaria compromisos'), 0, 0, 'C');
    //$this->Cell(10, 10, $EjercicioPpto, 0, 0, 'C'); 
//$this->Cell(25, 10,'', 0, 1, 'C');
	///// PRUEBA ***********
	$this->SetFont('Arial', 'B', 8);
	...
	$this->Cell(25, 3, 'T. COMPROMETIDO', 1, 1, 'C', 1);
 ...
 if(($fieldet['partida']!=00) and (($cont1==0) or ($pCapturada!=$fieldet['partida']))){
  ...
  if($rwPar!=0){
   ...
   /// ********************************************************************************************** ///
	...
	$sCA1 = "SELECT 
				...
				FROM 
					...
					(b.ff_ejecucion='$EjercicioPpto')
				WHERE 
					b.CodAnteproyecto = '".$fieldCon['CodPresupuesto']."' and 
					b.CodOrganismo = '".$fieldCon['Organismo']."' and 
	...
	//for($y=0; $y<$rCA1; $y++){
		 ...
		 $montoCreditoA = $montoCreditoA + $fCA1['mm_monto'];
		 
		 //$totalCreditoA=$totalCreditoA+$montoCreditoA;
	 // }
   ...
   for($a=0; $a<$rwdet; $a++){
	...
	$montoDisponible = ($montoAjustado - $montoCompromiso)+$montoCreditoA;
   }
	...
	//$pdf->SetWidths(array(18, 75,25,25,25,25,25,25,25,25));
	$pdf->SetWidths(array(18, 75,25,25,25));
	//$pdf->SetAligns(array('C','L','R','R','R','R','R','R','R','R'));
	$pdf->SetAligns(array('C','L','R','R','R'));

/*$pdf->Row(array($codigo_partida,$fpar['denominacion'],$montoAprobado,$montoAjustado,$montoCreditoA,$montoCompromiso,$montoCausado,$montoPagado,$montoDisponible));*/
	$pdf->Row(array($codigo_partida,$fpar['denominacion'],$montoAprobado,$montoAjustado,$montoCompromiso));
  }
 }
//// --------------------------------------------------------------------------------------------------
	...
	if($rows2!=0){
	  ...
	  /// ********************************************************************************************** ///
		...
		if($rcag!=0){
		    ...
		    $montoCreditoA1 = $montoCreditoA1 + $fca['mm_monto'];
			 //$totalCreditoA=$totalCreditoA+$montoCreditoA1;
		}
	  ...
	  for($b=0; $b<$rwdet2; $b++){
		...
		$montoDisponible1 = ($montoAjustado1 - $montoCompromiso1)+$montoCreditoA1;
	   ...
	   $montoCreditoA1 = number_format($montoCreditoA1,2,',','.');	 
	  ...
	  $pdf->SetWidths(array(18, 75,25,25,25));
	  $pdf->SetAligns(array('C','L','R','R','R'));
	  /*$pdf->Row(array($codigo_partida,$fpar2['denominacion'],$montoAprobado1,$montoAjustado1,$montoCreditoA1,$montoCompromiso1,$montoCausado1,$montoPagado1,$montoDisponible1));*/

 	$pdf->Row(array($codigo_partida,$fpar2['denominacion'],$montoAprobado1,$montoAjustado1,$montoCompromiso1));
   }
  }
//// --------------------------------------------------------------------------------------------------
...
//// --------------------------------------------------------------------------------------------------
 if($fieldet['partida']!=00){
	...
	if($rCA!=0){
		$fCA = mysql_fetch_array($qCA);	  	     
		$montoCreditoA2 = $fCA['mm_monto'];	
		$totalCreditoA=$totalCreditoA+$montoCreditoA2;
	  //}
	}
	 ...
	 $montoDisponible2 = number_format(($fieldet['MontoAjustado'] - $fieldet['MontoCompromiso'])+$montoCreditoA2,2,',','.');
	 
	 
	 $montoCreditoA2 = number_format($montoCreditoA2 ,2,',','.');	  	
	 ...
	 $totalDisponible = ($totalMontoAjustado - $totalMontoCompromiso)+$totalCreditoA;
	 ...
	 $pdf->SetWidths(array(18, 75,25,25,25));
	 $pdf->SetAligns(array('C','L','R','R','R'));
	 $pdf->Row(array($fieldet['cod_partida'],$field['denominacion'],$montoAprobado2,$montoAjustado2,$montoCompromiso2));
 }
}
	///// *** Mostrar *** /////
	 $totalMontoAprobado =  number_format($totalMontoAprobado,2,',','.');
	 $totalMontoAjustado =  number_format($totalMontoAjustado,2,',','.');
	 $totalCreditoA		 =	number_format($totalCreditoA,2,',','.');
...
/*$pdf->Row(array('' ,'Total:',$totalMontoAprobado,$totalMontoAjustado,$totalCreditoA,$totalMontoCompromiso,$totalMontoCausado,$totalMontoPagado,$totalDisponible));*/

	$pdf->Row(array('' ,'Total:',$totalMontoAprobado,$totalMontoAjustado,$totalMontoCompromiso));
	/////
//---------------------------------------------------*/
/*$pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(202, 202, 202); $pdf->SetTextColor(0, 0, 0);
