...
?>
<form id="frmentrada" name="frmentrada" action="anteproyecto_editardet.php?accion=ActualizarAnte" method="POST" onsubmit="return verificarDatosgenerales(this,'Guardar')">
<?php
$limit=(int) $limit;
echo "
...
$sqlAnt="SELECT * FROM pv_antepresupuesto WHERE CodAnteproyecto='001'";
...
<tr>
  <td>
	...
	<li><a onclick="javascript:alert('&iexcl;Haga click en botón *GUARDAR REGISTRO* si realizó cambios o pasar a siguiente pestaña!');" href="#">Detalle de Presupuesto</a></li>
	</ul>
	</div>
  </td>
</tr>
...
<table width="800" class="tblForm">
	...
	<td></td>		
		...
		<td><? $ano = date(Y); // devuelve el año
			...
			<input title="A&ntilde;o de Presupuesto" name="ejercicioPpto" type="text" id="ejercicioPpto" size="3" value="<?=$fieldAnt[EjercicioPpto]?>"/>* 
			F.Creaci&oacute;n:<input name="fcreacion" type="text" id="fcreacion" size="8" value="<?=$fAnteproyecto?>" readonly /> 
			 Estado:<input name="estado" type="text" id="estado" size="11" value="<?=$fieldAnt[Estado]?>" readonly/></td>
	</tr>
	</table>
	<!---////////////////////////////////////////////////////////////////////////////////////////////////-->
	...
	<tr><td></td>
	    ...
	    <td colspan="2"><input name="fdesde" type="text" id="finicio" size="10" value="<?=$fInicio?>" maxlength="10"/>*<i>(dd-mm-aaaa)</i></td>
	</tr>	
	<tr><td></td>
		<td class="tagForm">F. Termino:</td>
	    <td colspan="2"><input name="fhasta" type="text" id="ftermino" size="10" value="<?=$fFin?>" maxlength="10"/>*<i>(dd-mm-aaaa)</i></td>
	</tr>	
	<tr><td></td>
	...
	<tr><td></td>
	  ...
	  <td ><input name="montoautori" type="text" id="montoautori" size="20" maxlength="15" value="<?=$fieldAnt[MontoPresupuestado]?>"/>*<em>(99999999,99)</em></td>
	</tr>
	<tr><td></td>
	  <td class="tagForm">Monto Restante:</td><? $sql="SELECT MontoRestante FROM pv_anteproyecto WHERE CodAnteproyecto='".$fieldAnt['CodAnteproyecto']."'"; 
	                                             $qry=mysql_query($sql) or die ($sql.mysql_error());
												 if(mysql_num_rows($qry)!=0){
												   $fieldMontoR=mysql_fetch_array($qry);
												 }
												 ?>
	  <td><input name="montorestante" id="montorestante" type="text" size="20" maxlength="15" value="<?=$fieldMontoR[MontoRestante]?>" readonly/> <em>(99999999,99)</em></td>
	</tr>
	<tr><td></td></tr>
	<tr><td></td>
		...
		<? 
           echo"<td class='tagForm'>&Uacute;ltima Modif.:</td>
	           <td>
	             ...
	             <input name='ult_fecha' type='text' id='ult_fecha' size='25' value='".$fieldAnt['UltimaFecha']."' readonly />
	           </td>";
	   ?>
  </tr>
</table>
...
<!--<input name="btcontinuar" type="button" id="btcontinuar" value="Continuar" onclick="cargarPagina(this.form,'anteproyecto_editardet.php')"/>-->
...
function verificarDatosgenerales(formulario) {
		   ...
		   if (formulario.ejercicioPpto.value.length <1) {
		     alert(" Si desea modificar algun Registro debe completar el llenado del formulario.");
