...
function ULTIMO_SUELDO_NORMAL_DIARIO($_ARGS) {
	...
	if($field['TotalIngresos'] > $field2['TotalIngresos'])
	{
		...
		$montoTotal = $field['TotalIngresos'];

	} else {

		$montoTotal = $field2['TotalIngresos'];
	}
...
//	print_r ($_ARGS['PROCESO']);
	$sql = "SELECT TotalIngresos 
			FROM pr_tiponominaempleado 
			INNER JOIN pr_tipoproceso ON pr_tiponominaempleado.CodTipoProceso = pr_tipoproceso.CodTipoProceso
			WHERE
				CodOrganismo = '".$_ARGS['ORGANISMO']."'  AND
				CodTipoNom = '".$_ARGS['NOMINA']."'  AND
				CodPersona = '".$_ARGS['TRABAJADOR']."'  
				AND pr_tipoproceso.CodTipoProceso = 'FIN' 
				 AND Periodo = '".$_ARGS['PERIODO']."'";
				 
	$query = mysql_query($sql) or die ($sql.mysql_error());
	if (mysql_num_rows($query) != 0) $field = mysql_fetch_array($query);
	
	//echo $field['TotalIngresos'];
	//echo "asdsadad";
	return $field['TotalIngresos'];
}

//
function SUELDO_INTEGRAL($_ARGS) {
	$proceso = (string)"PRQ";
 $sql = "SELECT pr_tiponominaempleado.Periodo,
pr_tiponominaempleado.CodPersona,
pr_tiponominaempleado.CodOrganismo,
pr_tiponominaempleado.CodTipoProceso,
pr_tiponominaempleado.TotalIngresos
FROM
pr_tiponominaempleado
where 
pr_tiponominaempleado.CodTipoNom='".$_ARGS['NOMINA']."'  
AND pr_tiponominaempleado.Periodo = '".$_ARGS['PERIODO']."' 
AND  pr_tiponominaempleado.CodPersona = '".$_ARGS['TRABAJADOR']."' 
AND  pr_tiponominaempleado.CodOrganismo = '".$_ARGS['ORGANISMO']."'
AND pr_tiponominaempleado.CodTipoProceso = '".$proceso."'";
	//echo $sql;			
	$query = mysql_query($sql) or die ($sql.mysql_error());
	$field = mysql_fetch_array($query);
	...
	if (mysql_num_rows($query) != 0) $field = mysql_fetch_array($query);
	//
	return $field['TotalIngresos']+SUELDO_BASICO($_ARGS);
}
