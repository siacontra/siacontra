...
$sql = "SELECT 
			...
			ptnec.Monto,
			(SELECT Monto FROM pr_tiponominaempleadoconcepto WHERE CodPersona = mp.CodPersona AND CodTipoNom = '".$ftiponom."' AND Periodo = '".$fperiodo."' AND CodTipoproceso = '".$ftproceso."' AND (CodConcepto = '0111' or CodConcepto = '0205' or CodConcepto = '0210')) AS Aporte
		FROM
			mastpersonas mp
			INNER JOIN pr_tiponominaempleado ptne ON (mp.CodPersona = ptne.CodPersona)
			INNER JOIN pr_tiponominaempleadoconcepto ptnec ON (ptne.CodPersona = ptnec.CodPersona AND ptne.CodTipoNom = ptnec.CodTipoNom AND ptne.Periodo = ptnec.Periodo AND ptne.CodTipoproceso = ptnec.CodTipoProceso AND (ptnec.CodConcepto = '0106' or ptnec.CodConcepto = '0211' or ptnec.CodConcepto = '0212'))
...
while ($field = mysql_fetch_array($query)) {
	...
	$pdf->Row(array(number_format($field['Ndocumento'], 0, '', '.'), utf8_decode($field['Busqueda']), number_format($field['SueldoBasicoMes'], 2, ',', '.'), number_format($field['Monto'], 2, ',', '.'), number_format($field['Aporte'], 2, ',', '.'), number_format($montoenterar, 2, ',', '.')));
	if ($pdf->GetY() > 250) Cabecera($pdf, $ftiponom, $field_nomina['Nomina'], $field_proceso['Descripcion'], $periodo, $periodo_fecha);
}
//---------------------------------------------------
$pdf->SetFont('Arial', 'B', 8);
$pdf->Row(array('', 'TOTAL', number_format($sum_ingresos, 2, ',', '.'), number_format($sum_retenciones, 2, ',', '.'), number_format($sum_aportes, 2, ',', '.'), number_format($sum_totalenterar, 2, ',', '.'),));
//---------------------------------------------------
