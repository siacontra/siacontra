...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Ejecuci&oacute;n de Procesos (Pre N&oacute;mina) </td>
...
if ($filtrar == "DEFAULT") {
	...
	$sql = "SELECT FechaDesde, FechaHasta FROM pr_procesoperiodoprenomina WHERE CodOrganismo = '".$forganismo."' AND CodTipoNom = '".$ftiponom."' AND Periodo = '".$fperiodo."' AND CodTipoProceso = '".$ftproceso."'";
	...
	$sql = "SELECT 
			...
			WHERE 
				(me.CodPersona NOT IN (SELECT 
											CodPersona 
									   FROM 
									   		pr_tiponominaempleadoprenomina 
									   WHERE 
	...
	$sql = "SELECT 
			...
			FROM 
				...
				INNER JOIN rh_puestos rp ON (me.CodCargo=rp.CodCargo) 
			WHERE 
				(me.CodPersona IN (SELECT CodPersona 
								   ...
								   WHERE 
										...
										CodTipoProceso = '".$ftproceso."' AND 
										CodOrganismo = '".$forganismo."')) AND 
				(me.CodOrganismo = '".$forganismo."' AND me.CodTipoNom = '".$ftiponom."') $filtro_periodo 
			ORDER BY length(mp.Ndocumento) ASC, mp.Ndocumento ASC";
...
?>
<form id="frmentrada" name="frmentrada" action="ejecucion_procesos_prenomina.php" method="POST" onsubmit="return cargarDisponiblesProcesar(this.form);">
...
<table width="1000" class="tblFiltro">
    <tr>
        ...
        <td>
			...
			<select name="ftiponom" id="ftiponom" class="selectBig" onchange="getFOptions_PeriodoPreNomina(this.id, 'fperiodo', 'chkperiodo', document.getElementById('forganismo').value); getFOptions_Proceso('ftiponom', 'ftproceso', 'chktproceso', this.value, document.getElementById('forganismo').value);">
    ...
    <tr>
        ...
        <td>
			...
			<select name="fperiodo" id="fperiodo" style="width:90px;" onchange="getFOptions_ProcesoPreNomina(this.id, 'ftproceso', 'chktproceso', document.getElementById('ftiponom').value, document.getElementById('forganismo').value);">
				<option value=""></option>
				<?=getPeriodos($fperiodo, $ftiponom, $forganismo, 5);?>
			</select>
		</td>
        ...
        <td>
			...
			<select name="ftproceso" id="ftproceso" class="selectBig">
				<option value=""></option>
                <?=getTipoProcesoNomina($ftproceso, $fperiodo, $ftiponom, $forganismo, 5)?>
			</select>
		</td>
    </tr>
   ...
   <tr>
      <td>
		...
		<td>
			...
			<input type="button" value="Generar" style="width:90px;" onclick="procesarPreNomina(this.form);" />
			<table align="center"><tr><td><div style="background:#F9F9F9; height:350px; overflow: scroll; width:500px;">         
			<table width="1000" class="tblLista">
				<tr class="trListaHead">
					<th scope="col" width="25">&nbsp;</th>
