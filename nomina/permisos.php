...
echo "
<form name='frmentrada' action='permisos.php?filtro=".$_GET['filtro']."' method='POST' onsubmit='return false'>
...
<center><input type='button' name='btBuscar' value='Buscar' onclick='filtroPermisos(this.form, ".$_GET['limit'].", \"permisos.php\");'></center>
...
<table width="900" class="tblBotones">
  <tr>
    ...
    <td align="right">
		<input name="btNuevo" type="button" class="btLista" id="btNuevo" value="Nuevo" onclick="cargarPagina(this.form, 'permisos_nuevo.php');" />
		<input name="btEditar" type="button" class="btLista" id="btEditar" value="Editar" onclick="cargarOpcion(this.form, 'permisos_editar.php', 'SELF');" />
		...
		<input name="btPDF" type="button" class="btLista" id="btPDF" value="PDF" onclick="cargarVentana(this.form, 'permisos_pdf.php', 'height=800, width=800, left=200, top=200, resizable=yes');" /> |  
		<input name="btImprimir" type="button" class="btLista" id="btImprimir" value="Imprimir" onclick="abrirPermiso(this.form);" />
		<input name="btAnular" type="button" class="btLista" id="btAnular" value="Anular" onclick="anularPermiso(this.form);" />
	</td>
  </tr>
</table>
...
<table width="900" class="tblLista">
  <tr class="trListaHead">
	...
	if ($registros!=0) {
		//	CONSULTO LA TABLA
		$sql="SELECT rh_permisos.CodPermiso, mastempleado.CodEmpleado, mastpersonas.NomCompleto, rh_permisos.TipoFalta, rh_permisos.FechaIngreso, (SELECT mastmiscelaneosdet.Descripcion FROM mastmiscelaneosdet WHERE mastmiscelaneosdet.CodMaestro='PERMISOS' AND mastmiscelaneosdet.CodDetalle=rh_permisos.TipoPermiso) AS TipoPermiso, (SELECT mastmiscelaneosdet.Descripcion FROM mastmiscelaneosdet WHERE mastmiscelaneosdet.CodMaestro='TIPOFALTAS' AND mastmiscelaneosdet.CodDetalle=rh_permisos.TipoFalta) AS TipoFalta, rh_permisos.Estado, seguridad_alterna.CodOrganismo FROM rh_permisos, mastempleado, mastpersonas, seguridad_alterna WHERE (rh_permisos.CodPersona=mastpersonas.CodPersona) AND (mastpersonas.CodPersona=mastempleado.CodPersona) AND (mastempleado.CodDependencia=seguridad_alterna.CodDependencia AND seguridad_alterna.CodAplicacion='".$_SESSION["APLICACION_ACTUAL"]."' AND seguridad_alterna.Usuario='".$_SESSION["USUARIO_ACTUAL"]."' AND seguridad_alterna.FlagMostrar='S') ".$_GET['filtro']." ORDER BY rh_permisos.CodPermiso LIMIT ".$_GET['limit'].", $MAXLIMIT";
		$query=mysql_query($sql) or die ($sql.mysql_error());
