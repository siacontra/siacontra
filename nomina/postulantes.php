...
if ($_GET['accion']=="ELIMINAR") {
	$sql="DELETE FROM rh_postulantes WHERE Postulante='".$_POST['registro']."'";
...
//
echo "
<form name='frmentrada' action='postulantes.php?filtro=".$_GET['filtro']."' method='POST' onsubmit='return false'>
...
<table width='1000' class='tblFiltro'>
	...
	<tr>
		<td align='right'>
			Estado:
			<input type='checkbox' name='chkstatus' value='1' $obj[29] onclick='enabledStatus(this.form);' /> 
		</td>
		<td>
			<select name='fstatus' id='fstatus' class='selectMed' $obj[30]>
				<option value=''></option>";
				getStatusPostulante($obj[31], 0);
				echo "
			</select>
		</td>
...
$sql="SELECT rh_postulantes.* $varinstruccion $varcargo $varidioma $varcursos $varanios FROM rh_postulantes $tblinstruccion $tblcargos $tblidioma $tblcursos $tblanios WHERE (rh_postulantes.Postulante<>'') $coninstruccion $concargo $conidioma $concursos $conanios ".$_GET['filtro'];
$query=mysql_query($sql) or die ($sql.mysql_error());
...
<td align="right">
		<input name="btNuevo" type="button" class="btLista" id="btNuevo" value="Nuevo" onclick="cargarPagina(this.form, 'postulantes_nuevo.php');" />
		<input name="btEditar" type="button" class="btLista" id="btEditar" value="Editar" onclick="cargarOpcion(this.form, 'postulantes_editar.php?accion=EDITAR', 'SELF');" />
		<input name="btVer" type="button" class="btLista" id="btVer" value="Ver" onclick="cargarOpcion(this.form, 'postulantes_ver.php', 'BLANK', 'height=625, width=1000, left=0, top=0, resizable=yes');" />
		<input name="btEliminar" type="button" class="btLista" id="btEliminar" value="Eliminar" onclick="eliminarRegistro(this.form, 'postulantes.php?accion=ELIMINAR&limit=0', '1', 'POSTULANTES');" />
		<input name="btPDF" type="button" class="btLista" id="btPDF" value="PDF" onclick="cargarVentana(this.form, 'postulantes_pdf.php', 'height=800, width=800, left=200, top=200, resizable=yes');" /> | 
		<input name="btCarta" type="button" class="btLista" id="btCarta" value="Prep. Carta" />
	</td>
</tr>
</table>
...
if ($registros!=0) {
	//	CONSULTO LA TABLA
	$sql="SELECT rh_postulantes.* $varinstruccion $varcargo $varidioma $varcursos $varanios FROM rh_postulantes $tblinstruccion $tblcargos $tblidioma $tblcursos $tblanios WHERE (rh_postulantes.Postulante<>'') $coninstruccion $concargo $conidioma $concursos $conanios ".$_GET['filtro']." LIMIT ".$_GET['limit'].", $MAXLIMIT";
	$query=mysql_query($sql) or die ($sql.mysql_error());
	...
	for ($i=0; $i<$rows; $i++) {
		...
		echo "
		<tr class='trListaBody' onclick='mClk(this, \"registro\");' onmouseover='mOvr(this);' onmouseout='mOut(this);' id='".$field['Postulante']."'>
