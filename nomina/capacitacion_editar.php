...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Capacitaci&oacute;n | Actualizaci&oacute;n</td>
...
<form id="frmentrada" name="frmentrada" action="<?=$regresar?>.php" method="POST" onsubmit="return verificarCapacitacion(this, 'ACTUALIZAR');">
...
<table width="900" align="center">
  <tr>
		<td>
			...
			<ul>
			<!-- CSS Tabs -->
			<? if($tab1){?><li><a onclick="document.getElementById('tab1').style.display='block'; document.getElementById('tab2').style.display='none'; document.getElementById('tab3').style.display='none';" href="#">Capacitaci&oacute;n</a></li><? }?>
			<? if($tab2){?><li><a onclick="document.getElementById('tab1').style.display='none'; document.getElementById('tab2').style.display='block'; document.getElementById('tab3').style.display='none';" href="#">Fundamentaci&oacute;n</a></li><? }?>
			<? if($tab3){?><li><a onclick="document.getElementById('tab1').style.display='none'; document.getElementById('tab2').style.display='none'; document.getElementById('tab3').style.display='block';" href="#">Participantes</a></li><? }?>
			<? if($tab4){?><li><a onclick="javascript:alert('&iexcl;DEBE GUARDAR PRIMERO LA CAPACITACI&Oacute;N!');" href="#">Horario</a></li><? }?>
...
<table width="900" class="tblForm">
	<tr>
		...
		<td>
			<select name="organismo" id="organismo" class="selectBig">
				<?php getOrganismos($field['CodOrganismo'], 0); ?>
			</select>*
		</td>
	...
	<tr>
		...
		<td>
			...
			<input name="bt_examinar" type="button" id="bt_examinar" value="..." onclick="cargarVentana(this.form, 'lista_empleados.php?limit=0&campo=1', 'height=500, width=900, left=200, top=200, resizable=yes');" />*
		</td>		
	...
	<tr>		
		...
		<td>
			...
			<input name="bt_examinar" type="button" id="bt_examinar" value="..." onclick="cargarVentana(this.form, 'lista_cursos.php?limit=0&campo=1', 'height=500, width=900, left=200, top=200, resizable=yes');" />*
		</td>
		<td class="tagForm">Tipo:</td>
		<td>
			<select name="tcurso" id="tcurso">
				<option value=""></option>
				<?php getMiscelaneos($field['TipoCurso'], "TIPOCURSO", 0); ?>
			</select>*
		</td>
	</tr>
	<tr>
		...
		<td colspan="3">
			...
			<input name="bt_examinar" type="button" id="bt_examinar" value="..." onclick="cargarVentana(this.form, 'lista_centros_estudios.php?limit=0&campo=1&flag_estudio=N&flag_curso=S', 'height=500, width=900, left=200, top=200, resizable=yes');" />*
		</td>
	</tr>
	<tr>
		...
		<td>
			<select name="estado" id="estado" class="selectMed" onchange="getOptions_3(this.id, 'municipio', 'ciudad');">
				<option value=""></option>
				<?php getEstados($field['CodEstado'], $field['CodPais'], 0); ?>
			</select>*
		</td>
	</tr>
	<tr>
		...
		<td>                                                                              
			<select name="municipio" id="municipio" class="selectMed" onchange="getOptions_2(this.id, 'ciudad');">
				<option value=""></option>
				<?php getMunicipios($field['CodMunicipio'], $field['CodEstado'], 0); ?>
			</select>*
		</td>
		...
		<td>
			<select name="ciudad" id="ciudad" class="selectMed">
				<option value=""></option>
				<?php getCiudades($field['CodCiudad'], $field['CodMunicipio'], 0); ?>
			</select>*
		</td>
	</tr>
	<tr>
		<td class="tagForm">Tel&eacute;fono:</td>
	    <td><input name="tel" type="text" id="tel" size="15" maxlength="15" value="<?=$field['TelefonoContacto']?>" /></td>
		<td class="tagForm">Aula:</td>
		<td><input name="aula" type="text" id="aula" size="10" maxlength="5" value="<?=$field['Aula']?>" /></td>
	</tr>
	<tr>
		<td class="tagForm">Expositor:</td>
	    <td colspan="3"><input name="expositor" type="text" id="expositor" size="75" maxlength="100" value="<?=$field['Expositor']?>" /></td>
	</tr>	
	<tr>
		...
		<td>
			<select name="tcapacitacion" id="tcapacitacion">
				<option value=""></option>
				<?php getTipoCapacitacion($field['TipoCapacitacion'], 0); ?>
			</select>*
		</td>
		...
		<td>
			<select name="modalidad" id="modalidad">
				<option value=""></option>
				<?php getMiscelaneos($field['Modalidad'], "MODACAPAC", 0); ?>
			</select>*
		</td>
	</tr>	
	<tr>
		<td class="tagForm">Vacantes:</td>
	    <td><input name="vacantes" type="text" id="vacantes" size="10" maxlength="3" value="<?=$field['Vacantes']?>" />
	    *</td>
	...
	<tr>
	    ...
	    <td>
	    	...
	    	<input name="fhasta" type="text" id="fhasta" size="15" maxlength="10" value="<?=$fhasta?>" />*<em>(dd-mm-yyyy)</em>
		</td>
	...
	<tr>
	    ...
	    <td><input name="costo" type="text" id="costo" size="30" dir="rtl" value="<?=$estimado?>" /><em>(999999,99)</em></td>
		<td class="tagForm">Monto Asumido:</td>
	    <td><input name="asumido" type="text" id="asumido" size="30" dir="rtl" value="<?=$maxasumido?>" /><em>(999999,99)</em></td>
	</tr>
	<tr>
		<td class="tagForm">Monto Utilizado:</td>
	    <td><input name="utilizado" type="text" id="utilizado" size="30" dir="rtl" value="<?=$asumido?>" /><em>(999999,99)</em></td>
...
<table width="900" class="tblForm">
	...
	<tr><td><textarea name="funda1" id="funda1" cols="200" rows="3"><?=$field['Fundamentacion1']?></textarea></td></tr>
	...
	<tr><td><textarea name="funda2" id="funda2" cols="200" rows="3"><?=$field['Fundamentacion2']?></textarea></td></tr>
	<tr><td>3. Dias y horario mas factibles para el dictado:</td></tr>
	<tr><td><textarea name="funda3" id="funda3" cols="200" rows="3"><?=$field['Fundamentacion3']?></textarea></td></tr>
	<tr><td>4. &iquest;Qu&eacute; se espera desp&uacute;es de la capacitaci&oacute;n?:</td></tr>
	<tr><td><textarea name="funda4" id="funda4" cols="200" rows="3"><?=$field['Fundamentacion4']?></textarea></td></tr>
	<tr><td>5. &iquest;C&oacute;mo hace hoy su trabajo?:</td></tr>
	<tr><td><textarea name="funda5" id="funda5" cols="200" rows="3"><?=$field['Fundamentacion5']?></textarea></td></tr>
	<tr><td>6. &iquest;Hay colaboradores dentro de la empresa que puedan dictar este curso?:</td></tr>
	<tr><td><textarea name="funda6" id="funda6" cols="200" rows="3"><?=$field['Fundamentacion6']?></textarea></td></tr>
	</tr>
</table>
</div>
...
<table align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td><iframe name="iParticipante" class="frameTab" style="height:600px; width:898px;" id="iParticipante" src="capacitacion_participantes.php?capacitacion=<?=$field['Capacitacion']?>&organismo=<?=$field['CodOrganismo']?>" /></td>
	</tr>
	</tr>
</table>
</div>
