...
elseif ($modulo == "interfase_cuentas_por_pagar") {
	...
	if ($accion == "calcular") {
		...
		$queryProveedor = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
		
		//$fieldProveedor = mysql_fetch_array($queryProveedor);
		
		//if (mysql_num_rows($query) != 0) die("Algunas obligaciones se encuentran actualmente transferidas a CxP<br />Debe anular las obligaciones transferidas para calcularlas nuevamente.");
		...
		/**************************************************
		 * Elimino todos los registros de obligaciones de CxP 
		 * que han sido anulados y estan transferedidos
		 * al igual que lg_distribucioncompromisos
		 * para cada uno de los proveedores con las especificaciones de 
		 * Tipo Nomina-Periodo-Proceso-Proveedor-Numero Documento
		 * **************************************************/	
		
		$cantProveedorEliminar = mysql_num_rows($queryProveedor);
		
		
		if ($cantProveedorEliminar > 0)
		{
			while ($fieldProveedor = mysql_fetch_array($queryProveedor)) {
					
				
				//	obligacion
				$sql = "DELETE FROM ap_obligaciones
						WHERE
							CodProveedor = '".$fieldProveedor['CodProveedor']."' AND
							CodTipoDocumento = '".$fieldProveedor['CodTipoDocumento']."' AND
							NroDocumento = '".$fieldProveedor['NroDocumento']."'";
				$query_delete = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
				//	cuentas
			...
			//$proveedorCalcular[$i] = $fieldProveedor['CodProveedor'];
			//echo $proveedorCalcular[$i];
			//$i++;	
		...
		/************************************
		 *BUSCO LOS TRANSFERIDOS NO ANULADOS 
		 * 
		 * ********************************/
		...
		$queryProveedorNoAnulado = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
		
		$proveedorCalcular = array(); 	
		$i = 0;
		
		while ($field2 = mysql_fetch_array($queryProveedorNoAnulado)) 
		{
			
			$proveedorCalcular [$i] = $field2['CodProveedor'];
			$i++;
		}
		
		//print_r($proveedorCalcular);
		...
		*/
		...
		if(count($proveedorCalcular) > 0)
		{
			if ($CodTipoProceso == "BVC")
			{
				$condicionProveedor = "tnec.CodPersona not in ('".implode("','",$proveedorCalcular)."') AND";
				$condicionProveedor2 = "tnec2.CodPersona not in ('".implode("','",$proveedorCalcular)."') AND";
			
			} else {
					
				$condicionProveedor = "o.CodPersona not in ('".implode("','",$proveedorCalcular)."') AND";
				$condicionProveedor2 = "o2.CodPersona not in ('".implode("','",$proveedorCalcular)."') AND";
			}
		...
		{
				die("Ya se han calculado las obligaciones pendientes");
		}
				...
				while ($field = mysql_fetch_array($query)) {
					...
					$sql = "INSERT INTO ap_distribucionobligacion
							SET
								CodProveedor = '".$field['CodProveedor']."',
								CodTipoDocumento = '".$field['CodTipoDocumento']."',
								CodPresupuesto 	= (select CodPresupuesto from pv_presupuesto where EjercicioPpto='".$anho."'),	
								NroDocumento = '".$field['NroDocumento']."',
								Monto = '".$field['Monto']."',
								CodCuenta = '".$field['CodCuenta']."',
								CodCuentaPub20 = '".$field['CodCuentaPub20']."',
					...
					if ($FlagCompromiso == "S") {
						...
						$sql = "INSERT INTO lg_distribucioncompromisos
								SET
									Anio = '".substr($PeriodoActual, 0, 4)."',
									CodOrganismo = '".$CodOrganismo."',
									CodPresupuesto = (SELECT CodPresupuesto FROM `pv_presupuesto` WHERE EjercicioPpto = '".substr($PeriodoActual, 0, 4)."'),
									CodProveedor = '".$field['CodProveedor']."',
									CodTipoDocumento = '".$field['CodTipoDocumento']."',
									NroDocumento = '".$field['NroDocumento']."',
