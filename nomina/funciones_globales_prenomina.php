...
function DIAS_FECHA($_DESDE, $_HASTA) {
	list($ad, $md, $dd)=SPLIT( '[/.-]', $_DESDE);
	list($ah, $mh, $dh)=SPLIT( '[/.-]', $_HASTA);
...
function SUELDO_BASICO($_ARGS) {
	...
	while ($field = mysql_fetch_array($query)) {
		...
		if ($field['Estado'] == "A") $dias = DIAS_FECHA($desde, $hasta); //$i++;
		else {
			if ($field['Fegreso'] < $_ARGS['DESDE']) $dias = 0;
			else $dias = DIAS_FECHA($_ARGS['DESDE'], $field['Fegreso']); //$i++;
		}
		
		$monto = ($field['SueldoBasico'] / $_PARAMETROS['MAXDIASMES']) * $dias;  //echo "$i = $monto | ";
		$sum_sueldo += $monto;
	}
	return $sum_sueldo;
}
...
function DIFERENCIA_SUELDO_BASICO($_ARGS) {
	...
	while ($field_nivelaciones = mysql_fetch_array($query_nivelaciones)) {
		...
		$diferencia = $field_nivelaciones['SueldoTemporal'] - $sueldo_basico;
		
		$dias = DIAS_FECHA($desde, $hasta);
		$monto = (($field_nivelaciones['SueldoTemporal']/$_PARAMETROS['MAXDIASMES']) * $dias) - (($field_sueldo['SueldoBasico']/$_PARAMETROS['MAXDIASMES']) * $dias);
		$sum_diferencia += $monto;
		
		/*if ($dias != $_ARGS['DIAS_PROCESO']) $sum_diferencia += ($monto / 30 * $_ARGS['DIAS_PROCESO']);
		else $sum_diferencia += $monto;*/
	}
...
function DIAS_JERARQUIA($_ARGS) {
	...
	while ($field = mysql_fetch_array($query)) {
		...
		if ($field['Grado'] == "96" || $field['Grado'] == "97" || $field['Grado'] == "98" || $field['Grado'] == "99") {
			$dias = DIAS_FECHA($desde, $hasta);
			$suma += $dias;
		}
	}
...
function NUMERO_LUNES_FECHA($_ARGS) {
	list($ap, $mp) = SPLIT('[/.-]', $_ARGS['PERIODO']); $periodo_inicio = "01-$mp-$ap";
	list($ae, $me, $de) = SPLIT('[/.-]', FECHA_EGRESO($_ARGS));
	$primer_dia_semana = DIA_DE_LA_SEMANA($periodo_inicio);
	$dia_semana = $primer_dia_semana;
	$dia_fin = (int) $de;
	$lunes = 0;
	
	for ($dia=1; $dia<=$dia_fin; $dia++) {
...
function MESES_ANTIGUEDAD($_ARGS) {
	$fingreso = FECHA_INGRESO($_ARGS);
	$periodo_actual = $_ARGS['PERIODO']."-30";
	list($anios, $meses, $dias) = TIEMPO_DE_SERVICIO(formatFechaDMA($fingreso), formatFechaDMA($periodo_actual));
...
function ULTIMA_ALICUOTA_VACACIONAL($_ARGS) {
	$_PARAMETROS = PARAMETROS();
	$_MONTO = ULTIMO_SUELDO_NORMAL_DIARIO($_ARGS) * $_PARAMETROS["PAGOVACA"] / 360;
