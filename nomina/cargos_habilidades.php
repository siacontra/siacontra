...
<table width="600" class="tblLista">
	...
	<?php
	$sw=0;
	$sql="SELECT *, (SELECT COUNT(Tipo) FROM rh_cargohabilidades WHERE (Tipo='H')) AS rsh, (SELECT COUNT(Tipo) FROM rh_cargohabilidades WHERE (Tipo='D')) AS rsd FROM rh_cargohabilidades WHERE CodCargo='".$codcargo."' ORDER BY Tipo, Secuencia";
	...
	while($field=mysql_fetch_array($query)) {
		...
		<tr class="trListaBody" onclick="mClk(this, 'sec');" id="<?=$field['Secuencia']?>">
			<? if ($field['Tipo']=="D" && $sw==0) {?> <td rowspan="<?=$field['rsd']?>"><?=($tipo)?></td><? $sw=1; }?>
			<? if ($field['Tipo']=="H" && $sw==1) {?> <td rowspan="<?=$field['rsh']?>"><?=($tipo)?></td><? $sw=0; }?>
