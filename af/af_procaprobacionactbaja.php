...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Desincorporación de Activos</td>
...
echo"<form name='frmentrada' id='frmentrada' action='af_procaprobacionactbaja.php?limit=0' method='POST'>
...
<table width="990" class="tblLista">
 <tr> <input type="hidden" id="registro" name="registro"/>
  ...
  <td width="843" align="right">
    <input type="button" name="btAprobar" id="btAprobar" value="Desincorporar" onclick="cargarOpcion(this.form,'af_procesoctivosdesincorporar.php?','BLANK','height=590, width=1000, left=200, top=40, resizable=yes')"/>
  </tr>
</table>
...
<thead>
  <tr class="trListaHead">
        ...
        <th width="30" scope="col">Dependencia</th>
		<th width="50" scope="col">Cod. Int</th>
		<th width="200" scope="col">Decripci&oacute;n Local</th>
 	    <th width="150" scope="col">Categor&iacute;a</th>
        <th width="50" scope="col">Monto Local</th>
        <th width="100" scope="col">Clasificaci&oacute;n</th>    	
        <th width="140" scope="col">Ubicación</th>        
  ...
  if($ra!=0){
   ...
   for($i=0;$i<$ra;$i++){
	 ...
	 /// -------------------------------------------
	 $s_UbicActivo = "select * from af_ubicaciones where  CodUbicacion= '".$fa['Ubicacion']."'";
	 $q_UbicActivo = mysql_query($s_UbicActivo) or die ($s_UbicActivo.mysql_error()) ;
	 $f_UbicActivo = mysql_fetch_array($q_UbicActivo);
	 /// -------------------------------------------
    ...
    echo"<tr class='trListaBody' onclick='mClk(this, \"registro\")| cargarDireccion(tabs,\"$valor\");' id='$id'>
		...
		<td align='left'>".$f_UbicActivo['Descripcion']."</td>
