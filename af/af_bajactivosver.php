...
list($activo, $organismo)=SPLIT('[|]',$_GET['registro']);
//// CONSULTA PRINCIPAL
$sa = "select * from af_transaccionbaja where Organismo = '$organismo' and Activo='$activo'";
...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Baja de Activos | Ver Transacci&oacute;n</td>
...
<tr>
<?
 $s_activo = "select Descripcion from af_activo where Activo='".$fa['Activo']."'";
 $q_activo = mysql_query($s_activo) or die ($s_activo.mysql_error());
 $f_activo = mysql_fetch_array($q_activo);
 
?>
   <td class="tagForm">Activo #:</td>
   <td width="360"><input type="text" id="nro_activo" name="nro_activo" size="14" value="<?=$fa['Activo'];?>" disabled/>
                   <input type="text" name="descripcion" id="descripcion" size="50" readonly/>
       <input type="button" id="btActivo" name="btActivo" value="..." onclick="cargarVentanaLista(this.form, 'af_selectoractivos.php?limit=0&campo=2&ventana=insertarActivo','height=550, width=900, left=200, top=100, resizable=yes');" disabled/></td>
...
<tr>
  ...
  <td><select id="tipobaja" name="tipobaja" class="selectSma">
       <option value=""></option>
       <?
         $st = "select * from af_tipotransaccion";
		 $qt = mysql_query($st) or die ($st.mysql_error());
		 $rt = mysql_num_rows($qt);
		 if($rt!=0) $ft= mysql_fetch_array($qt);
		 for($i=0; $i<$rt; $i++){
...
<tr>
  ...
  <td><input type="text" id="f_actual" name="f_actual" size="10" value="<?=date("d-m-Y");?>"/> Nro. Factura:<input type="text" id="nrofactura" name="nrofactura"/></td>
...
<tr>
  ...
  <td><input type="text" name="descripcionCorta" id="descripcionCorta" size="10" disabled value="<?=date("Y-m");?>"/> <input type="text" id="codVoucher" name="codvoucher" size="10" disabled/></td>
