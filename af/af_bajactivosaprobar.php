...
<table width="1000" class="tblLista">
 <tr> <input type="hidden" id="registro" name="registro"/>
  ...
  <td align="right"></td>
  <td align="right">
    <input type="button" name="btAprobar" id="btAprobar" class="btLista" value="Aprobar" onclick="cargarPaginaAgregar(this.form, 'af_bajactivosaprobado.php?regresar=af_bajactivoslistar&fOrganismo=<?=$fOrganismo;?>&fDependencia=<?=$fDependencia;?>&fContabilidad=<?=$fContabilidad;?>&fActivo=<?=$fActivo;?>&fPeriodo=<?=$fPeriodo;?>&fFecha=<?=$fFecha;?>&fEstado=<?=$fEstado;?>');"/>
  </tr>
</table>
