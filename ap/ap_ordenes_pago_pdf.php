...
$sql = "SELECT
		...
		WHERE
			opc.Anio = SUBSTRING(NOW(), 1, 4) AND
...
function hoja_cuenta($pdf, $query_cuenta, $ln, $imprimir) {
	...
	if ($imprimir) {
		while($field_cuenta = mysql_fetch_array($query_cuenta)) {
			/*if ($field_cuenta['TipoSaldo'] == "D") { $debe = $field_cuenta['Monto']; $haber = 0.00; }
			...
			$pdf->Cell(115, $h, strtoupper(substr(utf8_decode($field_cuenta['Descripcion']), 0, 60)), 0, 0, 'L');
...
class PDF extends FPDF {
	...
	function Footer() {
		...
		global $field_orden;
		##
		if ($field_conforme['CargoConformeEncargado'] != "") $cargo_conforme = $field_conforme['CargoConformeEncargado'];
		else $cargo_conforme = $field_conforme['CargoConforme'];
		##
		if ($field_paguese['CargoPagueseEncargado'] != "") $cargo_paguese = $field_paguese['CargoPagueseEncargado'];
		else $cargo_paguese = $field_paguese['CargoPaguese'];
		##
		if ($_HOJA == "ORDEN DE PAGO") {
			...
			$this->SetXY(10, 242); $this->MultiCell(65, 4, $field_paguese['NomPaguese'], 0, 'L');
			$this->SetXY(75, 242); $this->MultiCell(65, 4, $field_conforme['NomConforme'], 0, 'L');
			$this->SetXY(140, 242); $this->MultiCell(65, 4, $field_conforme['NomProveedorPagar'], 0, 'L');			
			$this->SetXY(10, 246); $this->MultiCell(65, 4, $cargo_paguese, 0, 'C');
			$this->SetXY(75, 246); $this->MultiCell(65, 4, $cargo_conforme, 0, 'C');
			$this->SetXY(140, 246); $this->MultiCell(65, 4, '', 0, 'C');
			##
			$this->SetXY(10, 267);
			$this->SetFont('Arial', 'B', 9);
			$this->Cell(65, 4, 'FIRMA Y SELLO', 0, 0, 'C');
			$this->Cell(65, 4, 'FIRMA Y SELLO', 0, 0, 'C');
			$this->Cell(65, 4, 'FIRMA', 0, 1, 'C');
		}
	}
}
//---------------------------------------------------
...
$pdf->SetFont('Arial', 'B', 9);
$pdf->Cell(40, $h, 'POR CONCEPTO DE:', 0, 0, 'L');
$pdf->SetFont('Arial', '', 9);
$pdf->MultiCell(155, $h, utf8_decode($field_orden['Concepto']), 0, 'J');
$y+=$h*5;
