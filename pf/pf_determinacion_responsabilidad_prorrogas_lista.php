...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pf_determinacion_responsabilidad_prorrogas_lista" method="post">
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
			<input type="button" id="btNuevo" value="Nuevo" style="width:75px; <?=$btNuevo?>" onclick="cargarPagina(this.form, 'gehen.php?anz=pf_determinacion_responsabilidad_prorrogas_form&opcion=nuevo&action=pf_determinacion_responsabilidad_prorrogas_lista');" />
            
			<input type="button" id="btModificar" value="Modificar" style="width:75px; <?=$btModificar?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=determinacion_responsabilidad_prorrogas_modificar', 'gehen.php?anz=pf_determinacion_responsabilidad_prorrogas_form&opcion=modificar&action=pf_determinacion_responsabilidad_prorrogas_lista', 'SELF', '');" />
            
			<input type="button" id="btVer" value="Ver" style="width:75px; <?=$btVer?>" onclick="cargarOpcion2(this.form, 'pf_determinacion_responsabilidad_prorrogas_form.php?opcion=ver', 'BLANK', 'height=600, width=900, left=100, top=0, resizable=no', $('#registro').val());" />
            
			<input type="button" id="btRevisar" value="Revisar" style="width:75px; <?=$btRevisar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_determinacion_responsabilidad_prorrogas_form&opcion=revisar&action=pf_determinacion_responsabilidad_prorrogas_lista', 'SELF', '', 'registro');" />
            
			<input type="button" id="btAprobar" value="Aprobar" style="width:75px; <?=$btAprobar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_determinacion_responsabilidad_prorrogas_form&opcion=aprobar&action=pf_determinacion_responsabilidad_prorrogas_lista', 'SELF', '', 'registro');" />
            
			<input type="button" id="btAnular" value="Anular" style="width:75px; <?=$btAnular?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=determinacion_responsabilidad_prorrogas_anular', 'gehen.php?anz=pf_determinacion_responsabilidad_prorrogas_form&opcion=anular&action=pf_determinacion_responsabilidad_prorrogas_lista', 'SELF', '');" />
		</td>
	</tr>
</table>
...
<table width="100%" class="tblLista">
	...
	$sql = "SELECT
			...
			FROM
				pf_determinacionprorroga p
				INNER JOIN pf_determinacion vj ON (p.CodDeterminacion = vj.CodDeterminacion)
