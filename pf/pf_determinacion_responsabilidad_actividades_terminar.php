<?php
list($CodDeterminacion, $CodActividad) = split("[.]", $registro);
$sql = "SELECT
			vj.CodDeterminacion,
		...
		FROM
			pf_determinacion vj
			INNER JOIN pf_determinaciondetalle vjd ON (vj.CodDeterminacion = vjd.CodDeterminacion)
		...
		WHERE
			vj.CodDeterminacion = '".$CodDeterminacion."' AND
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pf_determinacion_responsabilidad_actividades" method="POST" onsubmit="return determinacion_responsabilidad_actividades_terminar(this);">
...
<table width="800" class="tblForm">
	<tr>
		<td class="tagForm" width="100">Determinaci&oacute;n:</td>
		<td>
        	<input type="hidden" id="Secuencia" value="<?=$field['Secuencia']?>" />
        	<input type="text" id="CodDeterminacion" value="<?=$field['CodDeterminacion']?>" style="width:110px;" class="codigo" disabled="disabled" /> - 
