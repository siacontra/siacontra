	...
	if ($opcion != "nuevo") {
		...
		$query_actividades = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
			...
			if ($field_actividades['Estado'] == "TE")
				$tiempo = $field_actividades['ProrrogaAcu'] + $field_actividades['DiasCierre'] + $field_actividades['ProrrogaPen'] ;
			else
				$tiempo = $field_actividades['ProrrogaAcu'] + $field_actividades['Duracion'] + $field_actividades['ProrrogaPen'] ;
			...
			<tr class="trListaBody" id="<?=$field_actividades['CodActividad']?>">
                ...
                <td align="center">
                    <?=intval($field_actividades['ProrrogaAcu'])?>
                    <input type="hidden" name="ProrrogaAcu" value="<?=($field_actividades['ProrrogaAcu'])?>" />
                ...
                <td align="center">
					...
					/*if ($field_actividades['Estado'] == "EJ") {
						...
						if($field_actividades['ProrrogaPen']!='' or $field_actividades['ProrrogaPen']!=0)
						echo $field_actividades['ProrrogaPen'];
						else echo 0;
					//}
					?>
                </td>
			...
			if ($field_actividades['FlagNoAfectoPlan'] == "N") {
				...
				$total_prorroga += $field_actividades['ProrrogaAcu'];
				$fase_prorroga += $field_actividades['ProrrogaAcu'];
