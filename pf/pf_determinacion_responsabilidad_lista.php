<?php
if ($lista == "todos") {
	$titulo = "Lista de Determinacion de Responsabilidades";
	$btRevisar = "display:none;";
	$btAprobar = "display:none;";
	$btGenerar = "display:none;";
}
elseif ($lista == "revisar") {
	$titulo = "Revisar Determinacion de Responsabilidades";
	$btModificar = "display:none;";
	$btAprobar = "display:none;";
	$btGenerar = "display:none;";
}
elseif ($lista == "aprobar") {
	$titulo = "Aprobar Determinacion de Responsabilidades";
	$btModificar = "display:none;";
	$btRevisar = "display:none;";
	$btGenerar = "display:none;";
}
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pf_determinacion_responsabilidad_lista" method="post">
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
			<input type="button" id="btModificar" value="Modificar" style="width:75px; <?=$btModificar?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=determinacion_responsabilidad_modificar', 'gehen.php?anz=pf_determinacion_responsabilidad_form&opcion=modificar&action=pf_determinacion_responsabilidad_lista', 'SELF', '');" />
            
			<input type="button" id="btVer" value="Ver" style="width:75px; <?=$btVer?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_determinacion_responsabilidad_form&opcion=ver', 'BLANK', 'height=700, width=900, left=100, top=0, resizable=no', $('#registro').val());" /> | 
            
			<input type="button" id="btRevisar" value="Revisar" style="width:75px; <?=$btRevisar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_determinacion_responsabilidad_form&opcion=revisar&action=pf_determinacion_responsabilidad_lista', 'SELF', '', 'registro');" />
            
			<input type="button" id="btAprobar" value="Aprobar" style="width:75px; <?=$btAprobar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_determinacion_responsabilidad_form&opcion=aprobar&action=pf_determinacion_responsabilidad_lista', 'SELF', '', 'registro');" />
            
			<input type="button" id="btCerrar" value="Cerrar" style="width:75px; <?=$btCerrar?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=determinacion_responsabilidad_cerrar', 'gehen.php?anz=pf_determinacion_responsabilidad_form&opcion=cerrar&action=pf_determinacion_responsabilidad_lista', 'SELF', '');" />
            
			<input type="button" id="btGenerar" value="Generar" style="width:75px; <?=$btGenerar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_determinacion_responsabilidad_form&opcion=generar&action=pf_determinacion_responsabilidad_lista', 'SELF', '', 'registro');" />
		</td>
	</tr>
</table>
...
<table width="1900" class="tblLista">
	...
	while ($field = mysql_fetch_array($query)) {
		...
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=$field['CodDeterminacion']?>">
